...
template <class T> struct para_sparse_vector {
  public:
    ...
    virtual std::vector<int> get_global_array() const {
    ...
    virtual T &operator()(const std::vector<int> &orbs) {
        return get_local_element(orbs[0]);
    }
    ...
    unmap_local_index(int i) const = 0; // { assert(false); };
};
...
template <class T> class para_array_0d : public para_sparse_vector<T> {
  public:
    ...
    const std::vector<int> unmap_local_index(int i) const {
        return store.unmap_local_index(i);
    }
    ...
    T &get(const std::vector<int> &orbs) { return store(0); }
 ...
 * the null objects; set_indices then sets up the array by telling it which
 * indices will contain LARGE objects. The non-null element indices are then
 * stored in global_indices/local_indices.
 *
 * The parallelism works via local_indices being different from global_indices.
 * all processors storing the same parallel array have the same global_indices,
 * but may have different local_indices. The local_indices are partitioned
 * equally amongst the processors in set_indices.
 *
 * The index maps store -1 if the index is invalid, otherwise they store the
 * value of the index. e.g. if an array holds elements 2, 3, then index_map[0] =
 * -1, but index_map[3] = 3.
 */
/// TODO: there is a lot of common ground between the different array classes:
...
template <class T> class para_array_1d : public para_sparse_vector<T> {
  public:
    ...
    const T &operator()(int i, int j = -1, int k = -1, int l = -1) const {
        ...
        if (is_distributed()) /**< if storage is distributed, make sure index is
                                 available on current processor */
    ...
    T &get(const std::vector<int> &orbs) { return store[orbs[0]]; }
    ...
    friend std::ostream &operator<<(std::ostream &os, para_array_1d &op) {
        for (int i = 0; i < op.max_size(); ++i)
            os << op.store[i] << std::endl;
        return os;
    }
  ...
  private:
    ...
    template <class Archive>
    void serialize(Archive &ar, const unsigned int version) {
        ar &stored_local &global_indices &global_indices_map &local_indices
            &local_indices_map &store;
    }
    ...
    bool stored_local;    /**< are all elements stored locally, or are they
                             distributed */
};

inline int tristore_2d(int i) { return i * (i + 1) / 2; }
...
inline int trimap_2d(int i, int j, int length, bool ut = false) {
    if (i >= j) {
        ...
        if (i >= halflen && j >= halflen)
            ...
            return tristore_2d(length - halflen - 1) + length - halflen +
                   tristore_2d(i) + j;
        else {
            int base = tristore_2d(length - halflen - 1) + length - halflen +
                       tristore_2d(halflen);
...
inline int squaremap(int i, int j, int length) { return i * length + j; }

/// parallel 2d lower triangular array class
/**
 * This has a flat layout; the corresponding 1d storage can the be distributed
 * over different processors. See para_array_1d and para_array_2d for more
 * details.
 */
template <class T> class para_array_triang_2d : public para_sparse_vector<T> {
  public:
    ...
    bool has(int i, int j, int k = -1, int l = -1) const {
    ...
    para_array_triang_2d<T> *clone() const {
        return new para_array_triang_2d<T>(*this);
    }
    ...
    void remove_local_index(int i, int j, int k = -1, int l = -1) {
        ...
        if (it == local_indices.end()) {
            cout << "Could not find operator of index " << i << "  " << j
                 << endl;
            cout << "Cannot remove the operator" << endl;
            exit(0);
        }
    ...
    void set_pair_indices(const std::vector<std::pair<int, int>> &occupied,
        ...
        for (std::vector<std::pair<int, int>>::const_iterator ptr =
                 occupied.begin();
  ...
  private:
    ...
    template <class Archive>
    void serialize(Archive &ar, const unsigned int version) {
        ar &stored_local &upper_triangular &length &global_indices
            &global_indices_map &local_indices &local_indices_map
                &global_index_pair &local_index_pair &store;
    }
...
template <class T> class para_array_2d : public para_sparse_vector<T> {
  public:
    ...
    /// query whether elements are non-null
    bool has(int i, int j, int k = -1) const {
    ...
    /// returns 1d index from i, j
    int squaremap(int i, int j) const { return ::squaremap(i, j, length); }
  ...
  private:
    ...
    void serialize(Archive &ar, const unsigned int version) {
        ar &stored_local &length &global_indices &global_indices_map
            &local_indices &local_indices_map &global_index_pair
                &local_index_pair &store;
    }
