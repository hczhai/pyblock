...
void SpinAdapted::InitBlocks::InitStartingBlock(
    StackSpinBlock &startingBlock, const bool &forward, int leftState,
    int rightState, const int &forward_starting_size,
    const int &backward_starting_size, const int &restartSize,
    const bool &restart, const bool &warmUp, int integralIndex,
    ...
    if (restart && restartSize != 1) {
        ...
        if (restart)
            StackSpinBlock::restore(forward, sites, startingBlock, leftState,
                                    rightState);
        else
            StackSpinBlock::restore(true, sites, startingBlock, leftState,
                                    rightState);
        startingBlock.set_twoInt(integralIndex);
    } else if (forward) {
        // startingBlock =
        // singleSiteBlocks[integralIndex][0];//StackSpinBlock(0,
        // forward_starting_size - 1, integralIndex, leftState==rightState,
        // true);

        if (startingBlock.nonactive_orb().size() != 0) {
            startingBlock = StackSpinBlock(
                0, forward_starting_size - 1, integralIndex,
                startingBlock.nonactive_orb(), leftState == rightState, true);
        } else
            startingBlock =
                StackSpinBlock(0, forward_starting_size - 1, integralIndex,
        ...
        if (dmrginp.add_noninteracting_orbs() &&
            ...
            newstartingBlock.default_op_components(false, true, true,
                                                   leftState == rightState);
            newstartingBlock.setstoragetype(LOCAL_STORAGE);

            if (braquanta.size() != 0)
                newstartingBlock.BuildSumBlock(
                    NO_PARTICLE_SPIN_NUMBER_CONSTRAINT, startingBlock,
                    dummyblock, braquanta, ketquanta);
            else
                newstartingBlock.BuildSumBlock(
                    NO_PARTICLE_SPIN_NUMBER_CONSTRAINT, startingBlock,
                    dummyblock);
            {
                long memoryToFree =
                    newstartingBlock.getdata() - startingBlock.getdata();
                ...
                Stackmem[omprank].deallocate(
                    newstartingBlock.getdata() + newsysmem, memoryToFree);
            }
...
void SpinAdapted::InitBlocks::InitNewSystemBlock(
    ...
    const int &sys_add, const bool &direct, int integralIndex,
    const Storagetype &storage, bool haveNormops, bool haveCompops,
    int constraint) {
    newSystem.set_integralIndex() = integralIndex;
    newSystem.default_op_components(direct, haveNormops, haveCompops,
                                    leftState == rightState);
...
void SpinAdapted::InitBlocks::InitNewSystemBlock(
    ...
    const int &sys_add, const bool &direct, int integralIndex,
    const Storagetype &storage, bool haveNormops, bool haveCompops,
    int constraint, const std::vector<SpinQuantum> &braquanta,
    const std::vector<SpinQuantum> &ketquanta) {
    newSystem.set_integralIndex() = integralIndex;
    newSystem.default_op_components(direct, haveNormops, haveCompops,
                                    leftState == rightState);
    newSystem.setstoragetype(storage);
    newSystem.BuildSumBlock(constraint, system, systemDot, braquanta,
                            ketquanta);
}

void SpinAdapted::InitBlocks::InitBigBlock(
    StackSpinBlock &leftBlock, StackSpinBlock &rightBlock, StackSpinBlock &big,
    const std::vector<SpinQuantum> &braquanta,
    const std::vector<SpinQuantum> &ketquanta) {
    ...
    if (dmrginp.hamiltonian() == BCS) {
        ...
        if (braquanta.size() != 0)
            big.BuildSumBlock(PARTICLE_SPIN_NUMBER_CONSTRAINT, leftBlock,
                              rightBlock, braquanta, ketquanta);
        else
            big.BuildSumBlock(PARTICLE_SPIN_NUMBER_CONSTRAINT, leftBlock,
                              rightBlock);
    }
...
void SpinAdapted::InitBlocks::InitNewEnvironmentBlock(
    StackSpinBlock &environment, StackSpinBlock &environmentDot,
    StackSpinBlock &newEnvironment, const StackSpinBlock &system,
    StackSpinBlock &systemDot, int leftState, int rightState,
    const int &sys_add, const int &env_add, const bool &forward,
    const bool &direct, const bool &onedot, const bool &nexact,
    const bool &useSlater, int integralIndex, bool haveNormops,
    bool haveCompops, const bool &dot_with_sys, int constraint) {
    ...
    if (forward) {
        ...
        environmentEnd = dmrginp.spinAdapted() ? dmrginp.last_site() - 1
                                               : dmrginp.last_site() / 2 - 1;
    } else {
        systemDotStart = dmrginp.spinAdapted()
    ...
    for (int i = 0; i < abs(environmentEnd - environmentStart) + 1; ++i)
        *(environmentSites.begin() + i) =
            min(environmentStart, environmentEnd) + i;
    ...
    if (useSlater) { // for FCI
        ...
        StateInfo tmp;
        TensorProduct(system_stateinfo, sysdot_stateinfo, tmp,
                      NO_PARTICLE_SPIN_NUMBER_CONSTRAINT);
        // tmp has the system+dot quantum numbers
        tmp.CollectQuanta();

        if (dmrginp.warmup() == LOCAL2 || dmrginp.warmup() == LOCAL3 ||
            dmrginp.warmup() == LOCAL4) {
            ...
            int environmentActiveEnd =
                forward ? environmentStart + nactiveSites - 1
                        : environmentStart - nactiveSites + 1;
            int environmentCoreStart =
                forward ? environmentActiveEnd + 1 : environmentActiveEnd - 1;

            std::vector<int> activeSites(nactiveSites), coreSites(ncoreSites);
            for (int i = 0; i < nactiveSites; ++i) {
                activeSites[i] =
                    min(environmentStart, environmentActiveEnd) + i;
            }
            ...
            if (coreSites.size() > 0) {
                ...
                environmentActive.default_op_components(
                    !forward, leftState == rightState);
                environmentActive.setstoragetype(DISTRIBUTED_STORAGE);
                environmentCore.default_op_components(!forward,
                                                      leftState == rightState);
                ...
                if ((!dot_with_sys && onedot) || !onedot) {
                    ...
                    environment.BuildSumBlock(constraint, environmentActive,
                                              environmentCore);
                } else {
                    newEnvironment.set_integralIndex() = integralIndex;
                    newEnvironment.default_op_components(
                        direct, haveNormops, haveCompops,
                        leftState == rightState);
                    newEnvironment.setstoragetype(DISTRIBUTED_STORAGE);
                    newEnvironment.BuildSumBlock(constraint, environmentActive,
                                                 environmentCore);
                    // p2out << "\t\t\t NewEnvironment block " << endl <<
                    // newEnvironment << endl;
                ...
                if ((!dot_with_sys && onedot) || !onedot) {
                    ...
                    environment.default_op_components(!forward,
                                                      leftState == rightState);
                    environment.setstoragetype(DISTRIBUTED_STORAGE);
                    environment.BuildTensorProductBlock(
                        environmentSites); // exact block
                } else {
                    newEnvironment.set_integralIndex() = integralIndex;
                    newEnvironment.default_op_components(
                        !forward, leftState == rightState);
            ...
            if (onedot)
                ...
                StateInfo environmentdot_stateinfo =
                    environmentDot.get_stateInfo();
            ...
            p2out << "\t\t\t Quantum numbers and states used for warm up :: "
                  << endl
                  << "\t\t\t ";
            ...
            if (dot_with_sys && onedot) {
                newEnvironment.set_integralIndex() = integralIndex;
                newEnvironment.BuildSlaterBlock(environmentSites,
                                                quantumNumbers, distribution,
                                                haveCompops, haveNormops);
            } else {
                environment.set_integralIndex() = integralIndex;
                environment.BuildSlaterBlock(environmentSites, quantumNumbers,
                                             distribution, haveCompops,
                                             haveNormops);
            }
        }
    } else {
        p2out << "\t\t\t Restoring block of size " << environmentSites.size()
              << " from previous iteration" << endl;

        if (dot_with_sys && onedot) {
            newEnvironment.set_integralIndex() = integralIndex;
            StackSpinBlock::restore(!forward, environmentSites, newEnvironment,
                                    leftState, rightState);
            ...
            environment.set_integralIndex() = integralIndex;
            StackSpinBlock::restore(!forward, environmentSites, environment,
                                    leftState, rightState);
    ...
    if (!dot_with_sys || !onedot) {
        ...
        newEnvironment.set_integralIndex() = integralIndex;
        newEnvironment.default_op_components(direct, haveNormops, haveCompops,
                                             leftState == rightState);
        newEnvironment.setstoragetype(DISTRIBUTED_STORAGE);
        ...
        // p2out << "\t\t\t NewEnvironment block " << endl << newEnvironment <<
        // endl; newEnvironment.printOperatorSummary();
    }
}
...
void SpinAdapted::InitBlocks::InitNewOverlapEnvironmentBlock(
    StackSpinBlock &environment, StackSpinBlock &environmentDot,
    StackSpinBlock &newEnvironment, const StackSpinBlock &system,
    StackSpinBlock &systemDot, int leftState, int rightState,
    const int &sys_add, const int &env_add, const bool &forward,
    int integralIndex, const bool &onedot, const bool &dot_with_sys,
    int constraint) {
    // now initialise environment Dot
    int systemDotStart, systemDotEnd, environmentDotStart, environmentDotEnd,
        environmentStart, environmentEnd;
    ...
    if (!dot_with_sys || !onedot) {
        ...
        // newEnvironment.set_op_array(OVERLAP) =
        // boost::shared_ptr<Op_component<Overlap> >(new
        // Op_component<Overlap>(false));
        newEnvironment.setstoragetype(DISTRIBUTED_STORAGE);
