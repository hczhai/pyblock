...
void StackSpinBlock::make_iterator(StackSpinBlock &b, opTypes op, int *data,
                                   int index, int numIndices) {

    std::vector<int> oneindex(numIndices, 0.0);
    std::vector<std::pair<int, int>> twoindex(numIndices,
    ...
    if (index == 1) {
        ...
        for (int i = 0; i < numIndices; i++)
            threeindex[std::make_tuple(data[4 * i], data[4 * i + 1],
...
void StackSpinBlock::restore(bool forward, const vector<int> &sites,
                             StackSpinBlock &b, int left, int right,
                             char *name) {
    dmrginp.diski->start();
    Timer disktimer;
    ...
    for (int i = 0; i < numthrds; i++) {
        if (forward)
            file[i] = str(boost::format("%s%s%d%s%d%s%d%s%d%s%d%s%d%d%s") %
                          dmrginp.save_prefix() % "/Block-f-sites-" % sites[0] %
                          "." % sites[sites.size() - 1] % "-states" % left %
                          "." % right % "-integral" % b.integralIndex % "rank" %
                          mpigetrank() % i % ".tmp");
        else
            file[i] = str(boost::format("%s%s%d%s%d%s%d%s%d%s%d%s%d%d%s") %
                          dmrginp.save_prefix() % "/Block-b-sites-" % sites[0] %
                          "." % sites[sites.size() - 1] % "-states" % left %
                          "." % right % "-integral" % b.integralIndex % "rank" %
                          mpigetrank() % i % ".tmp");
    }
    ...
    assert(fread(initialData, sizeof(int), 31, fp[0]) == 31);
    assert(fread(&allindexsize, sizeof(int), 1, fp[0]) == 1);
    int *allindices = new int[allindexsize]; // large data
    assert(fread(allindices, sizeof(int), allindexsize, fp[0]) == allindexsize);

    assert(fread(&(b.totalMemory), sizeof(long), 1, fp[0]) == 1);
    b.data = Stackmem[omprank].allocate(b.totalMemory);

    double walltime = globaltimer.totalwalltime();
    assert(fread(b.data, sizeof(double), b.totalMemory, fp[0]) ==
           b.totalMemory);

    fclose(fp[0]);

    pout << str(boost::format("Read  %-10.4fG of data in  %-10.4f s\n") %
                (b.totalMemory * sizeof(double) / 1.e9) %
                (globaltimer.totalwalltime() - walltime));
    ...
    if (numham != -1) {
        b.ops[HAM] = make_new_stackop(HAM, true);
    }
    if (numcre != -1) {
        b.ops[CRE] = make_new_stackop(CRE, true);
    }
    if (numcrecre != -1) {
        b.ops[CRE_CRE] = make_new_stackop(CRE_CRE, true);
    }
    if (numdesdescomp != -1) {
        b.ops[DES_DESCOMP] = make_new_stackop(DES_DESCOMP, true);
    }
    if (numcredes != -1) {
        b.ops[CRE_DES] = make_new_stackop(CRE_DES, true);
    }
    if (numcredescomp != -1) {
        b.ops[CRE_DESCOMP] = make_new_stackop(CRE_DESCOMP, true);
    }
    if (numcrecredescomp != -1) {
        b.ops[CRE_CRE_DESCOMP] = make_new_stackop(CRE_CRE_DESCOMP, true);
    }
    if (numdes != -1) {
        b.ops[DES] = make_new_stackop(DES, true);
    }
    if (numdesdes != -1) {
        b.ops[DES_DES] = make_new_stackop(DES_DES, true);
    }
    if (numcrecrecomp != -1) {
        b.ops[CRE_CRECOMP] = make_new_stackop(CRE_CRECOMP, true);
    }
    if (numdescre != -1) {
        b.ops[DES_CRE] = make_new_stackop(DES_CRE, true);
    }
    if (numdescrecomp != -1) {
        b.ops[DES_CRECOMP] = make_new_stackop(DES_CRECOMP, true);
    }
    if (numcredesdescomp != -1) {
        b.ops[CRE_DES_DESCOMP] = make_new_stackop(CRE_DES_DESCOMP, true);
    }
    if (numoverlap != -1) {
        b.ops[OVERLAP] = make_new_stackop(OVERLAP, true);
    }
    if (numcdd_sum != -1) {
        b.ops[CDD_SUM] = make_new_stackop(CDD_SUM, true);
    }
    if (numcdd_cd != -1) {
        b.ops[CDD_CRE_DESCOMP] = make_new_stackop(CDD_CRE_DESCOMP, true);
    }
    if (numcdd_dd != -1) {
        b.ops[CDD_DES_DESCOMP] = make_new_stackop(CDD_DES_DESCOMP, true);
    }
    if (numccd_sum != -1) {
        b.ops[CCD_SUM] = make_new_stackop(CCD_SUM, true);
    }
    if (numccd_cd != -1) {
        b.ops[CCD_CRE_DESCOMP] = make_new_stackop(CCD_CRE_DESCOMP, true);
    }
    if (numccd_cc != -1) {
        b.ops[CCD_CRE_CRECOMP] = make_new_stackop(CCD_CRE_CRECOMP, true);
    }
    if (numri3index != -1) {
        b.ops[RI_3INDEX] = make_new_stackop(RI_3INDEX, true);
    }
    if (numri4index != -1) {
        b.ops[RI_4INDEX] = make_new_stackop(RI_4INDEX, true);
    }

    if (b.localstorage)
        b.setstoragetype(LOCAL_STORAGE);
    else
        b.setstoragetype(DISTRIBUTED_STORAGE);

    // this should be in the same order as opTypes are written in enumerator.h
    // file
    //
    if (numham != -1) {
        make_iterator(b, HAM, &allindices[index], 1, numham);
        index += numham;
    }
    if (numcre != -1) {
        make_iterator(b, CRE, &allindices[index], 1, numcre);
        index += numcre;
    }
    if (numcrecre != -1) {
        make_iterator(b, CRE_CRE, &allindices[index], 2, numcrecre);
        index += 2 * numcrecre;
    }
    if (numdesdescomp != -1) {
        make_iterator(b, DES_DESCOMP, &allindices[index], 2, numdesdescomp);
        index += 2 * numdesdescomp;
    }
    if (numcredes != -1) {
        make_iterator(b, CRE_DES, &allindices[index], 2, numcredes);
        index += 2 * numcredes;
    }
    if (numcredescomp != -1) {
        make_iterator(b, CRE_DESCOMP, &allindices[index], 2, numcredescomp);
        index += 2 * numcredescomp;
    }
    if (numcrecredescomp != -1) {
        make_iterator(b, CRE_CRE_DESCOMP, &allindices[index], 1,
                      numcrecredescomp);
        index += numcrecredescomp;
    }
    if (numdes != -1) {
        make_iterator(b, DES, &allindices[index], 1, numdes);
        index += numdes;
    }
    if (numdesdes != -1) {
        make_iterator(b, DES_DES, &allindices[index], 2, numdesdes);
        index += 2 * numdesdes;
    }
    if (numcrecrecomp != -1) {
        make_iterator(b, CRE_CRECOMP, &allindices[index], 2, numcrecrecomp);
        index += 2 * numcrecrecomp;
    }
    if (numdescre != -1) {
        make_iterator(b, DES_CRE, &allindices[index], 2, numdescre);
        index += 2 * numdescre;
    }
    if (numdescrecomp != -1) {
        make_iterator(b, DES_CRECOMP, &allindices[index], 2, numdescrecomp);
        index += 2 * numdescrecomp;
    }
    if (numcredesdescomp != -1) {
        make_iterator(b, CRE_DES_DESCOMP, &allindices[index], 1,
                      numcredesdescomp);
        index += numcredesdescomp;
    }
    if (numoverlap != -1) {
        make_iterator(b, OVERLAP, &allindices[index], 1, numoverlap);
        index += numoverlap;
    }
    if (numcdd_sum != -1) {
        make_iterator(b, CDD_SUM, &allindices[index], 1, numcdd_sum);
        index += numcdd_sum;
    }
    if (numcdd_cd != -1) {
        make_iterator(b, CDD_CRE_DESCOMP, &allindices[index], 1, numcdd_cd);
        index += numcdd_cd;
    }
    if (numcdd_dd != -1) {
        make_iterator(b, CDD_DES_DESCOMP, &allindices[index], 1, numcdd_dd);
        index += numcdd_dd;
    }
    if (numccd_sum != -1) {
        make_iterator(b, CCD_SUM, &allindices[index], 1, numccd_sum);
        index += numccd_sum;
    }
    if (numccd_cd != -1) {
        make_iterator(b, CCD_CRE_DESCOMP, &allindices[index], 1, numccd_cd);
        index += numccd_cd;
    }
    if (numccd_cc != -1) {
        make_iterator(b, CCD_CRE_CRECOMP, &allindices[index], 1, numccd_cc);
        index += numccd_cc;
    }
    if (numri3index != -1) {
        b.ops[RI_3INDEX]->build_iterators(b, false);
    }
    if (numri4index != -1) {
        b.ops[RI_4INDEX]->build_iterators(b, false);
    }
    ...
    for (std::map<opTypes, boost::shared_ptr<StackOp_component_base>>::iterator
             it = b.ops.begin();
         it != b.ops.end(); ++it) {
        if (it->second->is_core() && it->first != RI_3INDEX &&
            it->first != RI_4INDEX) {
            ...
            for (int i = 0; i < it->second->get_size(); i++) {
                ...
                for (int j = 0; j < vecsize; j++) {
                    ...
                    localdata = it->second->get_local_element(i)[j]->allocate(
                        b.braStateInfo, b.ketStateInfo, localdata);
                }
            }
        }
    }
...
void StackSpinBlock::store(bool forward, const vector<int> &sites,
                           StackSpinBlock &b, int left, int right, char *name) {
    dmrginp.disko->start();
    Timer disktimer;
    std::string file[numthrds];
    ...
    for (int i = 0; i < numthrds; i++) {
        if (dmrginp.spinAdapted()) {
            if (forward)
                file[i] =
                    str(boost::format("%s%s%d%s%d%s%d%s%d%s%d%s%d%d%s") %
                        dmrginp.save_prefix() % "/Block-f-sites-" % sites[0] %
                        "." % sites[sites.size() - 1] % "-states" % left % "." %
                        right % "-integral" % b.integralIndex % "rank" %
                        mpigetrank() % i % ".tmp");
            ...
            if (forward)
                file[i] =
                    str(boost::format("%s%s%d%s%d%s%d%s%d%s%d%s%d%d%s") %
                        dmrginp.save_prefix() % "/Block-f-sites-" %
                        (sites[0] / 2) % "." % (sites[sites.size() - 1] / 2) %
                        "-states" % left % "." % right % "-integral" %
                        b.integralIndex % "rank" % mpigetrank() % i % ".tmp");
            else
                file[i] =
                    str(boost::format("%s%s%d%s%d%s%d%s%d%s%d%s%d%d%s") %
                        dmrginp.save_prefix() % "/Block-b-sites-" %
                        (sites[0] / 2) % "." % (sites[sites.size() - 1] / 2) %
                        "-states" % left % "." % right % "-integral" %
                        b.integralIndex % "rank" % mpigetrank() % i % ".tmp");
        }
    ...
    initialData[19] =
        b.has(CRE_CRE_DESCOMP) ? b.ops[CRE_CRE_DESCOMP]->size() : -1;
    initialData[20] =
        b.has(CRE_DES_DESCOMP) ? b.ops[CRE_DES_DESCOMP]->size() : -1;
    ...
    initialData[24] =
        b.has(CDD_CRE_DESCOMP) ? b.ops[CDD_CRE_DESCOMP]->size() : -1;
    initialData[25] =
        b.has(CDD_DES_DESCOMP) ? b.ops[CDD_DES_DESCOMP]->size() : -1;
    initialData[26] = b.has(CCD_SUM) ? b.ops[CCD_SUM]->size() : -1;
    initialData[27] =
        b.has(CCD_CRE_DESCOMP) ? b.ops[CCD_CRE_DESCOMP]->size() : -1;
    initialData[28] =
        b.has(CCD_CRE_CRECOMP) ? b.ops[CCD_CRE_CRECOMP]->size() : -1;
    ...
    allindices.insert(allindices.end(), b.complementary_sites.begin(),
                      b.complementary_sites.end());
...
void StackSpinBlock::sendcompOps(StackOp_component_base &opcomp, int I, int J,
                                 int optype, int compsite) {
#ifndef SERIAL
    boost::mpi::communicator world;
    std::vector<boost::shared_ptr<StackSparseMatrix>> oparray =
        opcomp.get_element(I, J);
    for (int i = 0; i < oparray.size(); i++) {
        calc.send(processorindex(compsite),
                  optype + i * 10 + 1000 * J + 100000 * I, *oparray[i]);

        // now broadcast the data
        // MPI::COMM_WORLD.Bcast(oparray[i]->get_data(),
        // oparray[i]->memoryUsed(), MPI_DOUBLE, trimap_2d(I, J,
        // dmrginp.last_site()));
        MPI_Send(oparray[i]->get_data(), oparray[i]->memoryUsed(), MPI_DOUBLE,
                 processorindex(compsite),
                 optype + i * 10 + 1000 * J + 100000 * I, Calc);
    }
#endif
}

void StackSpinBlock::recvcompOps(StackOp_component_base &opcomp, int I, int J,
                                 int optype) {
#ifndef SERIAL
    boost::mpi::communicator world;
    std::vector<boost::shared_ptr<StackSparseMatrix>> oparray =
        opcomp.get_element(I, J);
    for (int i = 0; i < oparray.size(); i++) {
        calc.recv(processorindex(trimap_2d(I, J, dmrginp.last_site())),
                  optype + i * 10 + 1000 * J + 100000 * I, *oparray[i]);
        ...
        // now broadcast the data
        MPI_Recv(oparray[i]->get_data(), oparray[i]->memoryUsed(), MPI_DOUBLE,
                 processorindex(trimap_2d(I, J, dmrginp.last_site())),
                 optype + i * 10 + 1000 * J + 100000 * I, Calc,
                 MPI_STATUS_IGNORE);
    }
#endif
    ...
    if (!ops[CRE]->is_local()) {
        for (int i = 0; i < get_sites().size(); i++) {
            if (ops[CRE]->has(sites[i])) {
                ...
                // now broadcast the data
                MPI_Bcast(op->get_data(), op->memoryUsed(), MPI_DOUBLE,
                          processorindex(sites[i]), Calc);
            }
        }
    }
...
void StackSpinBlock::addOneIndexOps() {
    ...
    dotindice.push_back(
        (sites[0] == 0) ? complementary_sites[0]
                        : complementary_sites[complementary_sites.size() - 1]);
    if (!dmrginp
             .spinAdapted()) { // when non-spinadapted, sites are spin orbitals
        dotindice.push_back(
            (sites[0] == 0)
                ? complementary_sites[1]
                : complementary_sites[complementary_sites.size() - 2]);
    }
            ...
            if (!ops[CRE_CRE_DESCOMP]->is_local()) {
                ...
                if (ops[CRE_CRE_DESCOMP]->has(I)) {
                    if (fromproc != mpigetrank())
                        ops[CRE_CRE_DESCOMP]->add_local_indices(I);

                    std::vector<boost::shared_ptr<StackSparseMatrix>> oparray =
                        ops[CRE_CRE_DESCOMP]->get_element(I);
                    for (int iproc = 0; iproc < oparray.size(); iproc++) {
                        ...
                        // now broadcast the data
                        MPI_Bcast(oparray[iproc]->get_data(),
                                  oparray[iproc]->memoryUsed(), MPI_DOUBLE,
                                  fromproc, Calc);
                    }
                }
            }
        ...
        if (has(CRE_DES_DESCOMP)) {
            if (!ops[CRE_DES_DESCOMP]->is_local()) {
                ...
                if (ops[CRE_DES_DESCOMP]->has(I)) {
                    if (fromproc != mpigetrank())
                        ops[CRE_DES_DESCOMP]->add_local_indices(I);

                    std::vector<boost::shared_ptr<StackSparseMatrix>> oparray =
                        ops[CRE_DES_DESCOMP]->get_element(I);
                    for (int iproc = 0; iproc < oparray.size(); iproc++) {
                        // this only broadcasts the frame but no data
                        mpi::broadcast(calc, *oparray[iproc], fromproc);
        ...
        for (int idx1 = idx; idx1 < dotindice.size(); ++idx1) {
            ...
            if (has(CRE_DESCOMP)) {
                if (!ops[CRE_DESCOMP]->is_local()) {
                    ...
                    if (ops[CRE_DESCOMP]->has(I, J)) {
                        ...
                        std::vector<boost::shared_ptr<StackSparseMatrix>>
                            oparray = ops[CRE_DESCOMP]->get_element(I, J);
                        for (int iproc = 0; iproc < oparray.size(); iproc++) {
                            // this only broadcasts the frame but no data
                            mpi::broadcast(calc, *oparray[iproc], fromproc);

                            // now allocate the data when it is not already
                            // there
                            if (fromproc != mpigetrank()) {
                                double *data = Stackmem[omprank].allocate(
                                    oparray[iproc]->memoryUsed());
                    ...
                    if (ops[DES_DESCOMP]->has(I, J)) {
                        ...
                        std::vector<boost::shared_ptr<StackSparseMatrix>>
                            oparray = ops[DES_DESCOMP]->get_element(I, J);
                        for (int iproc = 0; iproc < oparray.size(); iproc++) {
                            // this only broadcasts the frame but no data
                            mpi::broadcast(calc, *oparray[iproc], fromproc);
                    ...
                    // take the DCComp(I,I) and CCComp(I,I) where I=dot index
                    // and broadcast it
                    if (has(DES)) {
                        if (ops[CRE_CRECOMP]->has(I, J)) {
                            ...
                            std::vector<boost::shared_ptr<StackSparseMatrix>>
                                oparray = ops[CRE_CRECOMP]->get_element(I, J);
                            for (int iproc = 0; iproc < oparray.size();
                                 iproc++) {
                                // this only broadcasts the frame but no data
                                mpi::broadcast(calc, *oparray[iproc], fromproc);
                        ...
                        if (ops[DES_CRECOMP]->has(I, J)) {
                            ...
                            std::vector<boost::shared_ptr<StackSparseMatrix>>
                                oparray = ops[DES_CRECOMP]->get_element(I, J);
                            for (int iproc = 0; iproc < oparray.size();
                                 iproc++) {
                                // this only broadcasts the frame but no data
                                mpi::broadcast(calc, *oparray[iproc], fromproc);
                ...
                for (int i = 0; i < complementary_sites.size(); i++) {
                    ...
                    if (std::find(dotindice.begin(), dotindice.end(),
                                  compsite) != dotindice.end())
                        continue;
                    int I = (compsite > dotopindex) ? compsite : dotopindex;
                    int J = (compsite > dotopindex) ? dotopindex : compsite;

                    if (processorindex(compsite) ==
                            processorindex(trimap_2d(I, J, length)) ||
                        ops[CRE_DESCOMP]->is_local())
                        continue;

                    if (processorindex(compsite) == mpigetrank()) {
                        // this will potentially receive some ops
                        bool other_proc_has_ops = true;
                        calc.recv(processorindex(trimap_2d(I, J, length)), 0,
                                  other_proc_has_ops);
                        ...
                        if (has(DES)) {
                            other_proc_has_ops = true;
                            calc.recv(processorindex(trimap_2d(I, J, length)),
                                      0, other_proc_has_ops);
                            if (other_proc_has_ops) {
                                ops[CRE_CRECOMP]->add_local_indices(I, J);
                                recvcompOps(*ops[CRE_CRECOMP], I, J,
                                            CRE_CRECOMP);
                            }
                            other_proc_has_ops = true;
                            calc.recv(processorindex(trimap_2d(I, J, length)),
                            ...
                            if (other_proc_has_ops) {
                                ops[DES_CRECOMP]->add_local_indices(I, J);
                                recvcompOps(*ops[DES_CRECOMP], I, J,
                                            DES_CRECOMP);
                            }
                        }
                    } else {
                        // this will potentially send some ops
                        if (processorindex(trimap_2d(I, J, length)) ==
                            mpigetrank()) {
                            bool this_proc_has_ops =
                                ops[CRE_DESCOMP]->has_local_index(I, J);
                            calc.send(processorindex(compsite), 0,
                                      this_proc_has_ops);
                            if (this_proc_has_ops) {
                                sendcompOps(*ops[CRE_DESCOMP], I, J,
                                            CRE_DESCOMP, compsite);
                            }
                            this_proc_has_ops =
                                ops[DES_DESCOMP]->has_local_index(I, J);
                            calc.send(processorindex(compsite), 0,
                                      this_proc_has_ops);
                            if (this_proc_has_ops) {
                                sendcompOps(*ops[DES_DESCOMP], I, J,
                                            DES_DESCOMP, compsite);
                            }
                            if (has(DES)) {
                                this_proc_has_ops =
                                    ops[CRE_CRECOMP]->has_local_index(I, J);
                                calc.send(processorindex(compsite), 0,
                                          this_proc_has_ops);
                                if (this_proc_has_ops) {
                                    sendcompOps(*ops[CRE_CRECOMP], I, J,
                                                CRE_CRECOMP, compsite);
                                }
                                this_proc_has_ops =
                                    ops[DES_CRECOMP]->has_local_index(I, J);
                                calc.send(processorindex(compsite), 0,
                                          this_proc_has_ops);
                                if (this_proc_has_ops) {
                                    sendcompOps(*ops[DES_CRECOMP], I, J,
                                                DES_CRECOMP, compsite);
                                }
                            }
                        } else
                            continue;
...
void StackSpinBlock::formTwoIndexOps() {
    ...
    for (int idx = 0; idx < dotindice.size(); ++idx)
        for (int i = 0; i < complementary_sites.size(); i++) {
            int dotopindex = dotindice[idx];
            int compsite = complementary_sites[i];
            if (find(dotindice.begin(), dotindice.end(), compsite) !=
                    dotindice.end() &&
                dotopindex > compsite) {
                continue;
            }
            ...
            if (dmrginp.use_partial_two_integrals() ||
                screen_cd_interaction(I, J, sites, *get_twoInt(), screen_tol))
                cdpair.push_back(std::pair<int, int>(I, J));

            if (dmrginp.use_partial_two_integrals() ||
                screen_dd_interaction(I, J, sites, *get_twoInt(), screen_tol))
    ...
    for (int idx = 0; idx < dotindice.size(); ++idx)
        for (int i = 0; i < complementary_sites.size(); i++) {
            ...
            if (dmrginp.use_partial_two_integrals() ||
                ...
                if (find(dotindice.begin(), dotindice.end(), I) !=
                        dotindice.end() &&
                    find(dotindice.begin(), dotindice.end(), J) !=
                        dotindice.end()) {
    ...
    for (int optypeindex = 0; optypeindex < opTypevec.size(); optypeindex++) {
...
#else
        {
            ...
            // for each processor loop over all its CDcomp operators and place
            // them in ops_On_proc
            int arraysize = 0;
            ...
            if (mpigetrank() == proc) {
                for (int i = 0; i < ops[optype]->get_size(); i++) {
                    ...
                    for (int j = 0; j < vecsize; j++)
                        ops_On_proc.push_back(
                            ops[optype]->get_local_element(i)[j]);
                }
                arraysize = ops_On_proc.size();
            }

#ifndef SERIAL
            // broadcast this ops_On_proc, so each processor has the shell of
            // the operators
            ...
            if (mpigetrank() != proc) {
                for (int i = 0; i < arraysize; i++) {
                    if (optype == CRE_DESCOMP)
                        ops_On_proc.push_back(
                            boost::shared_ptr<StackCreDesComp>(
                                new StackCreDesComp));
                    else if (optype == DES_DESCOMP)
                        ops_On_proc.push_back(
                            boost::shared_ptr<StackDesDesComp>(
                                new StackDesDesComp));
                    else if (optype == DES_CRECOMP)
                        ops_On_proc.push_back(
                            boost::shared_ptr<StackDesCreComp>(
                                new StackDesCreComp));
                    else if (optype == CRE_CRECOMP)
                        ops_On_proc.push_back(
                            boost::shared_ptr<StackCreCreComp>(
                                new StackCreCreComp));
                }
            }
#endif
            ...
            // make these comp operators from normal operators using
            // multithreading SplitStackmem();
#pragma omp parallel for schedule(dynamic)
            for (int i = 0; i < ops_On_proc.size(); i++) {
                if (optype == CRE_DESCOMP)
                    ((StackCreDesComp &)(*ops_On_proc[i]))
                        .buildfromCreDes(*this);
                else if (optype == DES_DESCOMP)
                    ((StackDesDesComp &)(*ops_On_proc[i]))
                        .buildfromDesDes(*this);
                else if (optype == DES_CRECOMP)
                    ((StackDesCreComp &)(*ops_On_proc[i]))
                        .buildfromDesCre(*this);
                else if (optype == CRE_CRECOMP)
                    ((StackCreCreComp &)(*ops_On_proc[i]))
                        .buildfromCreCre(*this);
            }

            for (int i = 0; i < ops_On_proc.size(); i++) {

#ifndef SERIAL
                MPI_Allreduce(MPI_IN_PLACE, ops_On_proc[i]->get_data(),
                              ops_On_proc[i]->memoryUsed(), MPI_DOUBLE, MPI_SUM,
                              Calc);
#endif
...
void StackSpinBlock::addAllCompOps() {
    ...
    for (int optypeindex = 0; optypeindex < opTypevec.size(); optypeindex++) {
...
#else
        {
            ...
            if (mpigetrank() != proc)
                for (int i = 0; i < arraysize; i++)
                    ops_On_proc.push_back(boost::shared_ptr<StackSparseMatrix>(
                        new StackSparseMatrix));
