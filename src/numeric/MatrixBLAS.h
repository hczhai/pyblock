...
double MatrixDotProduct(const T1 &a, const T2 &b) {
...
template <class T1, class T2>
void MatrixScaleAdd(double d, const T1 &aref, T2 &b) {
#ifdef BLAS
    // ugly hack, sometimes aref is a TransposeMatrix which does not have the
    // function Store() so I cast it as a Matrix and then use the usual Matrix
    // functions
...
void MatrixTensorProduct(const T1 &a_ref, char conjA, Real scaleA,
                         const T2 &b_ref, char conjB, Real scaleB, T3 &c,
    ...
    //  if ((brows == 1) && (bcols == 1))
    {
        ...
        if (conjA == 'n') {
            ...
            for (int i = 0; i < a.Nrows(); i++)
                DAXPY(a.Ncols(), scaleA * scaleB * b00,
                      a.Store() + i * a.Ncols(), 1, cptr + i * c.Ncols(), 1);
            return;
        } else {
            double *aptr = a.Store();
            double *cptr = c.Store() + rowstride * c.Ncols() + colstride;
    ...
    try {
        if (conjA == 'n' && conjB == 'n') {
            ...
            for (int i = 0; i < aRows; ++i)
                for (int j = 0; j < aCols; ++j) {
                    ...
                    for (int k = 0; k < bRows; ++k)
                        DAXPY(bCols, scale, &b(k + 1, 1), 1,
                              &c(i * bRows + k + 1 + rowstride,
                                 j * bCols + 1 + colstride),
                              1);
                }
            return;
            ...
            for (int i = 0; i < aRows; ++i)
                for (int j = 0; j < aCols; ++j) {
                    Real scale = scaleA * scaleB * a(i + 1, j + 1);
                    for (int k = 0; k < bRows; ++k)
                        DAXPY(bCols, scale, &b(1, k + 1), bRows,
                              &c(i * bRows + k + 1 + rowstride,
                                 j * bCols + 1 + colstride),
                              1);
                }
            ...
            for (int i = 0; i < aRows; ++i)
                for (int j = 0; j < aCols; ++j) {
                    Real scale = scaleA * scaleB * a(j + 1, i + 1);
                    for (int k = 0; k < bRows; ++k)
                        DAXPY(bCols, scaleA * scaleB * a(j + 1, i + 1),
                              &b(1, k + 1), bRows,
                              &c(i * bRows + k + 1 + rowstride,
                                 j * bCols + 1 + colstride),
                              1);
        ...
        for (int i = 1; i <= A.Nrows(); ++i)
            for (int j = 1; j <= A.Ncols(); ++j)
                c.SubMatrix((i - 1) * B.Nrows() + 1, i * B.Nrows(),
                            (j - 1) * B.Ncols() + 1, j * B.Ncols()) +=
                    (scaleA * scaleB) * A(i, j) * B;
#endif
...
template <class T1, class T2>
void MatrixMultiply(double d, const T1 &a, T2 &b) {
    //  b += d * a;
#ifdef BLAS
    assert((a.Nrows() == b.Nrows()) && (a.Ncols() == b.Ncols()));
    int n = a.Nrows() * a.Ncols();
...
template <class T1, class T2, class T3>
void MatrixMultiply(const T1 &a, char conjA, const T2 &b, char conjB, T3 &c,
                    Real scale, double cfactor = 1.) {
...
#ifdef BLAS
            dgemm_(&conjA, &conjB, &bCols, &aRows, &bRows, &scale, b.Store(),
                   &bCols, a.Store(), &aCols, &cfactor, c.Store(), &bCols);
#else
            c += (scale * a) * b;
#endif
        } else if (conjA == 'n' && conjB == 't') {
...
#ifdef BLAS
            dgemm_(&conjB, &conjA, &bRows, &aRows, &bCols, &scale, b.Store(),
                   &bCols, a.Store(), &aCols, &cfactor, c.Store(), &bRows);
#else
            c += (scale * a) * b.t();
#endif
        } else if (conjA == 't' && conjB == 'n') {
...
#ifdef BLAS
            dgemm_(&conjB, &conjA, &bCols, &aCols, &bRows, &scale, b.Store(),
                   &bCols, a.Store(), &aCols, &cfactor, c.Store(), &bCols);
#else
            c += (scale * a.t()) * b;
#endif
...
#ifdef BLAS
            dgemm_(&conjB, &conjA, &bRows, &aCols, &bCols, &scale, b.Store(),
                   &bCols, a.Store(), &aCols, &cfactor, c.Store(), &bRows);
#else
            c += (scale * a.t()) * b.t();
#endif
...
void diagonalise_tridiagonal(std::vector<double> &diagonal,
                             std::vector<double> &offdiagonal, int numelements,
                             Matrix &vec);

void CatenateProduct(const ObjectMatrix<Matrix *> &a, Matrix &b,
                     bool allocate = false);
void CatenateProduct(const ObjectMatrix<StackMatrix *> &a, StackMatrix &b);
char TransposeOf(char c);
inline char TransposeOf(char c) {
    assert(c == 'n' || c == 't');
    return (c == 'n') ? 't' : 'n';
}
...
template <class T>
void CyclicPermute(std::vector<T> &v, std::vector<T> &result, int n);
...
template <class T> bool find(vector<T> &v, const T &value) {
    return std::find(v.begin(), v.end(), value) != v.end();
}

template <class T>
void get_sorted_indices(const std::vector<T> &data, std::vector<int> &indices) {
