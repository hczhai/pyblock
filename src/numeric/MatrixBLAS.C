...
void SpinAdapted::xsolve_AxeqB(const Matrix &a, const ColumnVector &b,
                               ColumnVector &x) {
    FORTINT ar = a.Nrows();
    int bc = 1;
    int info = 0;
...
void SpinAdapted::svd(Matrix &M, DiagonalMatrix &d, Matrix &U, Matrix &V) {
    ...
    Mt = M.t();
    int info = 0;
    DGESVD('A', 'A', nrows, ncols, Mt.Store(), nrows, d.Store(), Ut.Store(),
           nrows, V.Store(), ncols, workspace, lwork, info);
...
void SpinAdapted::diagonalise_tridiagonal(std::vector<double> &diagonal,
                                          std::vector<double> &offdiagonal,
                                          int numelements, Matrix &vec) {
    int nrows = numelements;
    int ncols = numelements;

    vec.ReSize(nrows, nrows);

    Matrix vec_transpose;
    vec_transpose = vec;
    vector<double> workarray(4 * nrows - 2, 0);
    int info = 0;

    DSTEV('V', nrows, &(diagonal[0]), &(offdiagonal[0]), vec_transpose.Store(),
          nrows, &(workarray[0]), info);
...
void SpinAdapted::diagonalise(Matrix &sym, DiagonalMatrix &d, Matrix &vec) {
    ...
    int query = -1;
    DSYEV('V', 'L', nrows, workmat.Store(), nrows, dptr, &(workquery[0]), query,
          info); // do query to find best size

    int optlength = static_cast<int>(workquery[0]);
    vector<double> workspace(optlength);

    DSYEV('V', 'U', nrows, workmat.Store(), nrows, dptr, &(workspace[0]),
          optlength, info); // do query to find best size
...
void SpinAdapted::diagonalise(StackMatrix &sym, DiagonalMatrix &d) {
    ...
    int query = -1;
    DSYEV('V', 'L', nrows, &workmat[0], nrows, dptr, &(workquery[0]), query,
          info); // do query to find best size

    int optlength = static_cast<int>(workquery[0]);
    vector<double> workspace(optlength);

    DSYEV('V', 'U', nrows, &workmat[0], nrows, dptr, &(workspace[0]), optlength,
          info); // do query to find best size
...
void SpinAdapted::CatenateProduct(const ObjectMatrix<Matrix *> &a, Matrix &b,
                                  bool allocate) {
    try {
        ...
        for (int i = 0; i < indexRows.size(); ++i) {
            indexRows[i] =
                (i > 0) ? a(i - 1, 0)->Nrows() + indexRows[i - 1] : 1;
            rowLength += a(i, 0)->Nrows();
        }
        for (int i = 0; i < indexCols.size(); ++i) {
            indexCols[i] =
                (i > 0) ? a(0, i - 1)->Ncols() + indexCols[i - 1] : 1;
            colLength += a(0, i)->Ncols();
        }

        if (!allocate)
            assert(b.Nrows() == rowLength &&
                   b.Ncols() == colLength); // precondition
        else
        ...
        for (int i = 0; i < a.Nrows(); ++i)
            for (int j = 0; j < a.Ncols(); ++j) {
                ...
                int bcols = b.Ncols();
                double *bptr =
                    b.Store() + bcols * (indexRows[i] - 1) + (indexCols[j] - 1);
                Matrix *aij = a(i, j);
...
#else
                b.SubMatrix(indexRows[i], indexRows[i] + a(i, j)->Nrows() - 1,
                            indexCols[j], indexCols[j] + a(i, j)->Ncols() - 1) =
                    *(a(i, j));
#endif
            }
    } catch (Exception) {
...
void SpinAdapted::CatenateProduct(const ObjectMatrix<StackMatrix *> &a,
                                  StackMatrix &b) {
    try {
        std::vector<int> indexRows(a.Nrows());
        std::vector<int> indexCols(a.Ncols());
