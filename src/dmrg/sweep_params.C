...
SpinAdapted::SweepParams::SweepParams() {
    ...
    if (dmrginp.spinAdapted())
        n_iters = (dmrginp.last_site() - 2 * forward_starting_size - sys_add -
                   env_add) /
                      sys_add +
                  1;
    else
        n_iters = (dmrginp.last_site() - 4 * forward_starting_size -
                   2 * sys_add - 2 * env_add) /
                      (2 * sys_add) +
                  1;
...
void SpinAdapted::SweepParams::set_sweep_parameters() {
    ...
    */
    ...
    if (dmrginp.get_twodot_method() == 1) {
        ...
        if (this->get_sweep_iter() > 0 &&
            (this->get_sweep_iter() < dmrginp.twodot_to_onedot_iter() ||
             dmrginp.algorithm_method() == TWODOT))
            this->set_additional_noise() =
                0.5 * dmrginp.get_twodot_gamma() * this->get_largest_dw();
    }

    noise = dmrginp.sweep_noise_schedule()[current];
    additional_noise =
        this->get_additional_noise(); // dmrginp.get_twodot_noise();

    p1out << "\n\t\t\t Sweep iteration ... "
          << SpinAdapted::SweepParams::sweep_iter;
    p1out << " " << SpinAdapted::SweepParams::keep_states << " "
          << SpinAdapted::SweepParams::davidson_tol << " "
          << SpinAdapted::SweepParams::noise << " "
          << SpinAdapted::SweepParams::additional_noise << endl;
    else pout << endl;
    ...
    if (dmrginp.algorithm_method() == ONEDOT) {
        ...
        env_add = 0;
        if (dmrginp.spinAdapted())
            n_iters = (dmrginp.last_site() - 2 * forward_starting_size -
                       sys_add - env_add) /
                          sys_add +
                      1;
        else
    ...
    if (dmrginp.algorithm_method() == TWODOT_TO_ONEDOT) {
        if (dmrginp.twodot_to_onedot_iter() <=
            SpinAdapted::SweepParams::sweep_iter) {
            onedot = true;
            env_add = 0;
            if (dmrginp.spinAdapted())
    ...
    // now figure out number of iterations and starting size, during first call
    // only
    p2out << "\t\t\t forward system starting size ... " << forward_starting_size
          << " " << n_iters << endl;
    p2out << "\t\t\t backward system starting size ... "
          << backward_starting_size << " " << n_iters << endl;
...
void SpinAdapted::SweepParams::savestate(const bool &forward, const int &size) {
    if (mpigetrank() == 0) {
        char file[5000];
        sprintf(file, "%s%s%d%s", dmrginp.save_prefix().c_str(), "/statefile.",
                mpigetrank(), ".tmp");
...
void SpinAdapted::SweepParams::restorestate(bool &forward, int &size) {
    if (mpigetrank() == 0) {
        char file[5000];
        sprintf(file, "%s%s%d%s", dmrginp.load_prefix().c_str(), "/statefile.",
                mpigetrank(), ".tmp");
