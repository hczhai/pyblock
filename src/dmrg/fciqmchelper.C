...
// assumes that the state has been canonicalized in the left canonical form
// already and stored on disk
MPS::MPS(int stateindex) {
    ...
    if (mpigetrank() == 0) {
        ...
        if (!dmrginp.spinAdapted()) {
            rotSites[0] = 0;
            rotSites[1] = 3; // this should be 1 for spinadapted
        } else {
            rotSites[0] = 0;
            rotSites[1] = 1; // this should be 1 for spinadapted
        }
        ...
        if (!dmrginp.spinAdapted()) {
            ...
            if (dmrginp.add_noninteracting_orbs() &&
                dmrginp.molecule_quantum().get_s().getirrep() != 0) {
                rotMat = std::vector<Matrix>(4, m);
                rotMat[0](1, 1) = 1.0;
                rotMat[1](1, 1) = 1.0;
                rotMat[2](1, 1) = 1.0;
...
// this is a helper function to make a MPS from a occupation number
...
void MPS::Init(std::vector<bool> &occnum) {
    ...
    for (int i = 0; i < MPS::sweepIters - 1; i++) {
        ...
        SpinQuantum sq[] = {sTotal};
        int qs[] = {1};
        int n = 1;
        StateInfo stateTotal(n, sq, qs), currentState;
        TensorProduct(
            stateTotal,
            const_cast<StateInfo &>(MPS::siteBlocks[i + 1].get_stateInfo()),
            currentState, NO_PARTICLE_SPIN_NUMBER_CONSTRAINT);
        std::vector<Matrix> rotMat;
        rotMat.resize(currentState.quanta.size(), dummy);

        int spinQuantumIndex = occnum[2 * i + 2] * 2 + occnum[2 * i + 3];

        SpinQuantum sqout;
        if (spinQuantumIndex == 0)
            sqout = sTotal;
        else if (spinQuantumIndex == 1) {
            assert(sTotal.totalSpin.getirrep() > 0);
            sqout =
                SpinQuantum(sTotal.particleNumber + 1,
                            SpinSpace(sTotal.totalSpin.getirrep() - 1),
                            (sTotal.orbitalSymmetry + SymmetryOfOrb(i + 1))[0]);
            ...
            sqout = SpinQuantum(
                sTotal.particleNumber + 2, sTotal.totalSpin,
                (((sTotal.orbitalSymmetry + SymmetryOfOrb(i + 1))[0]) +
                 SymmetryOfOrb(i + 1))[0]);
        }
        pout << currentState << endl;
        pout << sqout << endl;
        std::vector<SpinQuantum>::iterator it =
            find(currentState.quanta.begin(), currentState.quanta.end(), sqout);
        if (it == currentState.quanta.end()) {
            pout << "Something is probably wrong with the determinant string. "
                    "please check again."
                 << endl;
            exit(0);
        }
    ...
    TensorProduct(stateTotal,
                  const_cast<StateInfo &>(
                      MPS::siteBlocks[MPS::sweepIters].get_stateInfo()),
                  secondLastState, NO_PARTICLE_SPIN_NUMBER_CONSTRAINT);
    int index1 =
        occnum[2 * MPS::sweepIters] * 2 + occnum[2 * MPS::sweepIters + 1];

    if (dmrginp.spinAdapted() && secondLastState.quanta.size() <= 3 &&
        index1 > 1)
        index1--;

    // now make wavefunction with the big state A
    w.initialise(dmrginp.effective_molecule_quantum_vec(), secondLastState,
                 MPS::siteBlocks[MPS::sweepIters + 1].get_stateInfo(), true);

    int index2 =
        occnum[2 * MPS::sweepIters + 2] * 2 + occnum[2 * MPS::sweepIters + 3];
    if (dmrginp.spinAdapted() && index2 > 1)
        index2--;
...
// this is a helper function to make a MPS from a occupation number
// representation of determinant this representation is slightly different than
// the usual occupation, here each integer element is a spatial orbital which can
// have a value 0, -1, 1, or 2.
MPS::MPS(ulonglong *occnum, int length) {
    ...
    for (int i = 0; i < length; i++) {
        ...
        for (int j = 63; j >= 0; j--) {
            if (dmrginp.spinAdapted() && index >= 2 * dmrginp.last_site())
                break;
            if (!dmrginp.spinAdapted() && index >= dmrginp.last_site())
                break;
...
MPS::MPS(std::vector<bool> &occ) { Init(occ); }
...
void MPS::writeToDiskForDMRG(int stateindex, bool writeStateAverage) {
    ...
    TensorProduct(const_cast<StateInfo &>(MPS::siteBlocks[0].get_stateInfo()),
                  const_cast<StateInfo &>(MPS::siteBlocks[1].get_stateInfo()),
                  stateOfSites12, NO_PARTICLE_SPIN_NUMBER_CONSTRAINT);
    ...
    Matrix dummy, m(1, 1);
    m = 1.0;
    // first check if this MPS was produced by a determinant or read from DMRG
    // output if it is a determinant the first rotation matrix has some of the
    // ncols 0
    ...
    // if it was generated from a determinant we have to make a combined
    // rotation matrix for the first two sites
    if (mpigetrank() == 0) {
        Matrix dummy, m(1, 1);
        m = 1.0;
        ...
        if (isDeterminant) {
            ...
            for (int I = 0; I < stateOfSites12.quanta.size(); I++) {
                const std::vector<int> &oldToNewI =
                    stateOfSites12.oldToNewState.at(
                        I); // all the uncollected state
                int stateIndex = 0;
                for (int i = 0; i < oldToNewI.size(); i++) {
                    ...
                    if (SiteTensors[1][rightq].Ncols() !=
                        0) { // the right quanta is retained

                        // right q is from a dot and only has single quantas,
                        // further we assume that the only options are empty or
                        // closed
                        assert(MPS::siteBlocks[1]
                                   .get_stateInfo()
                                   .quanta[rightq]
                                   .get_s()
                                   .getirrep() == 0);
                        ...
                        if (Rotations12[I].Ncols() == 0) {
                            Rotations12[I] =
                                Matrix(stateOfSites12.quantaStates[I],
                                       SiteTensors[0][leftq].Ncols());
                            for (int c = 0; c < SiteTensors[0][leftq].Ncols();
                                 c++)
                                Rotations12[I](c + 1, c + 1 + stateIndex) = 1.0;
                        } else {
                            pout << "We cannot have multiple ways to arrive at "
                                    "the same quantum"
                                 << endl;
                            exit(0);
                        }
                    ...
                    stateIndex +=
                        MPS::siteBlocks[0].get_stateInfo().quantaStates[leftq] *
                        MPS::siteBlocks[1].get_stateInfo().quantaStates[rightq];
                }
            }
        }
        ...
        if (isDeterminant) {
            ...
            SpinAdapted::StateInfo::transform_state(Rotations12, stateOfSites12,
                                                    renormState);
        } else {
            SaveRotationMatrix(rotSites, SiteTensors[1], stateindex);
            if (writeStateAverage)
                SaveRotationMatrix(rotSites, SiteTensors[1], -1);
            SpinAdapted::StateInfo::transform_state(
                SiteTensors[1], stateOfSites12, renormState);
        }
        ...
        for (int i = 1; i < MPS::sweepIters - 1; i++) {
            ...
            SpinAdapted::StateInfo::transform_state(SiteTensors[i + 1],
                                                    currentState, renormState);
        }
        StateInfo bigState;
        TensorProduct(currentState,
                      const_cast<StateInfo &>(
                          MPS::siteBlocks[MPS::sweepIters + 1].get_stateInfo()),
...
double calculateOverlap(const MPS& statea, const MPS& stateb) {
  ...
  int leftState=0, rightState=1, forward_starting_size=1,
backward_starting_size=0, restartSize =0; InitBlocks::InitStartingBlock(system,
forward, leftState, rightState, forward_starting_size, backward_starting_size,
restartSize, restart, warmUp, 0);
  ...
  for (int i=0; i<MPS::sweepIters; i++) {
    ...
    SpinAdapted::operatorfunctions::TensorProduct(o, siteOverlap, &stateA,
&stateB, onew, 1.0, true); o = onew;
  }
  ...
  SpinAdapted::operatorfunctions::TensorMultiply(o, siteOverlap, &braStateInfo,
&ketStateInfo, stateb.getw(), temp, SpinQuantum(0, SpinSpace(0), IrrepSpace(0)),
true, 1.0); double overlap = DotProduct(statea.getw(), temp); return overlap;
}
*/

void calcHamiltonianAndOverlap(int statea, int stateb, double &h, double &o,
                               bool sameStates, int integralIndex) {

    StackSpinBlock system, siteblock;
    bool forward = true, restart = false, warmUp = false;
    int leftState = 0, rightState = 1, forward_starting_size = 1,
        backward_starting_size = 0, restartSize = 0;
    int statebindex = sameStates ? 0 : 1;
    InitBlocks::InitStartingBlock(system, forward, leftState, statebindex,
                                  forward_starting_size, backward_starting_size,
    ...
    */
    ...
    std::vector<int> rotSites(2, 0);
    int sweepIters = dmrginp.spinAdapted() ? dmrginp.last_site() - 2
                                           : dmrginp.last_site() / 2 - 2;
    ...
    for (int i = 0; i < sweepIters - 1; i++) {
        ...
        if (i < normToComp) {
            ...
            InitBlocks::InitNewSystemBlock(
                system, dotsite, newSystem, 0, statebindex, sys_add, direct,
                integralIndex, DISTRIBUTED_STORAGE, true, false);
        } else {
            pout << "comp ops " << endl;
            InitBlocks::InitNewSystemBlock(
                system, dotsite, newSystem, 0, statebindex, sys_add, direct,
                integralIndex, DISTRIBUTED_STORAGE, false, true);
        }
        newSystem.transform_operators(
            const_cast<std::vector<Matrix> &>(Rotationa),
            const_cast<std::vector<Matrix> &>(Rotationb));
        {
    ...
    StackSpinBlock dotsite2(sweepIters + 1, sweepIters + 1, integralIndex,
                            false);
    ...
    system.addAdditionalOps();
    InitBlocks::InitNewSystemBlock(system, dotsite1, newSystem, 0, statebindex,
                                   sys_add, direct, integralIndex,
