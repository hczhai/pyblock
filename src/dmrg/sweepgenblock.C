...
void SweepGenblock::BlockAndDecimate(
    SweepParams &sweepParams, StackSpinBlock &system, StackSpinBlock &newSystem,
    const bool &useSlater, const bool &dot_with_sys, int stateA, int stateB) {
    ...
    // bool doComp = true;
    InitBlocks::InitNewSystemBlock(system, systemDot, newSystem, stateA, stateB,
                                   sweepParams.get_sys_add(), dmrginp.direct(),
                                   system.get_integralIndex(),
                                   DISTRIBUTED_STORAGE, doNorms, doComp);
...
double SweepGenblock::do_one(SweepParams &sweepParams, const bool &warmUp,
                             const bool &forward, const bool &restart,
                             const int &restartSize, int stateA, int stateB) {
    Timer sweeptimer;
    int integralIndex = 0;
    ...
    pout << ((forward)
                 ...
                 : "\t\t\t Starting renormalisation sweep in backwards "
                   "direction")
         << endl;
    pout << "\t\t\t "
            "=================================================================="
    ...
    InitBlocks::InitStartingBlock(system, forward, stateA, stateB,
    ...
    StackSpinBlock::store(
        forward, system.get_sites(), system, stateA,
        stateB); // if restart, just restoring an existing block --
    sweepParams.savestate(forward, system.get_sites().size());
    bool dot_with_sys = true;
    if (restart)
        Sweep::set_dot_with_sys(dot_with_sys, system, sweepParams, forward);
    // Sweep::set_dot_with_sys(dot_with_sys, system,
    // const_cast<bool&>(forward));
    ...
    for (; sweepParams.get_block_iter() < sweepParams.get_n_iters();) {
        ...
        StackSpinBlock::store(forward, system.get_sites(), system, stateA,
                              stateB);
        pout << system << endl;
...
void SweepGenblock::do_one(SweepParams &sweepParams, const bool &forward,
                           int stateA, int stateB) {
    Timer sweeptimer;
    int integralIndex = 0;
    StackSpinBlock system;
    ...
    InitBlocks::InitStartingBlock(system, forward, stateA, stateB,
                                  sweepParams.get_forward_starting_size(),
                                  sweepParams.get_backward_starting_size(), 0,
                                  false, false, integralIndex);
    ...
    for (; sweepParams.get_block_iter() < sweepParams.get_n_iters();) {
        ...
        BlockAndDecimate(sweepParams, system, newSystem, false, dot_with_sys,
                         stateA, stateB);
...
void SweepGenblock::do_one_partialSweep(SweepParams &sweepParams,
                                        const bool &forward, int stateA,
                                        int stateB, int integralIndex) {
    Timer sweeptimer;
    StackSpinBlock system;
