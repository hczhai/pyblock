...
void SpinAdapted::Sweep::set_dot_with_sys(bool &dot_with_sys,
                                          const StackSpinBlock &system,
    ...
    if (dmrginp.algorithm_method() == TWODOT_TO_ONEDOT &&
        dmrginp.twodot_to_onedot_iter() % 2 == 0) {
        if (forward && !sweepParams.get_onedot() &&
            system.get_complementary_sites()[0] >= dmrginp.last_site() / 2 - 1)
            dot_with_sys = false;
        if (forward && sweepParams.get_onedot() &&
            system.get_complementary_sites()[0] >= dmrginp.last_site() / 2)
            dot_with_sys = false;
        if (!forward && (system.get_sites()[0] - 1 <= dmrginp.last_site() / 2))
            dot_with_sys = false;
    } else if (dmrginp.algorithm_method() == TWODOT_TO_ONEDOT &&
               dmrginp.twodot_to_onedot_iter() % 2 == 1) {
        if (forward &&
            system.get_complementary_sites()[0] >= dmrginp.last_site() / 2 - 1)
            dot_with_sys = false;
        if (!forward && !sweepParams.get_onedot() &&
            system.get_sites()[0] - 1 <= dmrginp.last_site() / 2)
            dot_with_sys = false;
        if (!forward && sweepParams.get_onedot() &&
            (system.get_sites()[0] - 1 < dmrginp.last_site() / 2))
            dot_with_sys = false;
...
void SpinAdapted::Sweep::makeSystemEnvironmentBigOverlapBlocks(
    const std::vector<int> &systemSites, StackSpinBlock &systemDot,
    StackSpinBlock &environmentDot, StackSpinBlock &system,
    StackSpinBlock &newSystem, StackSpinBlock &environment,
    StackSpinBlock &newEnvironment, StackSpinBlock &big,
    SweepParams &sweepParams, const bool &dot_with_sys, const bool &useSlater,
    ...
    if (systemSites.size() == 1) {
        ...
        InitBlocks::InitStartingBlock(system, forward, braState, braState,
                                      ...
                                      restartSize, restart, warmUp,
                                      integralIndex);
    } else {
        system.set_integralIndex() = integralIndex;
        StackSpinBlock::restore(forward, systemSites, system, braState,
                                ketState);
    }
    ...
    if (!sweepParams.get_onedot() || dot_with_sys) {
        ...
        if ((dmrginp.calc_type() == RESPONSE ||
            ...
            system.get_sites()[0] != 0 &&
            system.get_sites()[0] >
                dmrginp.num_occupied_orbitals()) { // response and forward and
                                                   // after active sites
            dmrginp.set_molecule_quantum() =
                SpinQuantum(2, SpinSpace(0), IrrepSpace(0));
            newSystem.BuildSumBlock(PARTICLE_NUMBER_CONSTRAINT, system,
                                    systemDot);
        } else if ((dmrginp.calc_type() == RESPONSEAAAV) &&
                   system.get_sites()[0] != 0 &&
                   system.get_sites()[0] >
                       dmrginp
                           .num_occupied_orbitals()) { // response and forward
                                                       // and after active sites
            dmrginp.set_molecule_quantum() =
                SpinQuantum(1, SpinSpace(1), IrrepSpace(0));
            newSystem.BuildSumBlock(PARTICLE_NUMBER_CONSTRAINT, system,
                                    systemDot);
        } else if ((dmrginp.calc_type() == RESPONSEAAAC) &&
                   system.get_sites()[0] ==
                       0) { // response and forward and after active sites
            int coreOrbs = dmrginp.get_closedorbs().size();
            int totalN =
                moleculeQ.particleNumber + moleculeQ.totalSpin.getirrep();
            // int totalN = moleculeQ.particleNumber;
            int closedN = coreOrbs * 2;
            dmrginp.set_molecule_quantum() =
                SpinQuantum(totalN - closedN + 1, SpinSpace(1), IrrepSpace(0));
            newSystem.BuildSumBlock(PARTICLE_NUMBER_CONSTRAINT, system,
                                    systemDot);
        } else
    ...
    if (!dot_with_sys && sweepParams.get_onedot())
        InitBlocks::InitNewOverlapEnvironmentBlock(
            environment, systemDot, newEnvironment, system, systemDot, braState,
            ketState, sweepParams.get_sys_add(), sweepParams.get_env_add(),
        ...
        if ((dmrginp.calc_type() == RESPONSE ||
             dmrginp.calc_type() == RESPONSELCC) &&
            system.get_sites()[0] == 0 &&
            *system.get_sites().rbegin() >=
                dmrginp.num_occupied_orbitals()) { // response and forward and
                                                   // after active sites
            dmrginp.set_molecule_quantum() =
                SpinQuantum(2, SpinSpace(0), IrrepSpace(0));
            InitBlocks::InitNewOverlapEnvironmentBlock(
                environment, environmentDot, newEnvironment, system, systemDot,
                braState, ketState, sweepParams.get_sys_add(),
                sweepParams.get_env_add(), forward, integralIndex,
...
void SpinAdapted::Sweep::makeSystemEnvironmentBigBlocks(
    StackSpinBlock &system, StackSpinBlock &systemDot,
    StackSpinBlock &newSystem, StackSpinBlock &environment,
    StackSpinBlock &environmentDot, StackSpinBlock &newEnvironment,
    StackSpinBlock &big, SweepParams &sweepParams, const bool &dot_with_sys,
    const bool &useSlater, int integralIndex, int braState, int ketState,
    const vector<SpinQuantum> &braquanta,
    const vector<SpinQuantum> &ketquanta) {
    bool forward = (system.get_sites()[0] == 0);
    bool haveNormOps = dot_with_sys,
         haveCompOps = dmrginp.get_lowMemoryAlgorithm() ? !dot_with_sys : true;

    bool envnormops = !haveNormOps,
         envcompops = dmrginp.get_lowMemoryAlgorithm() ? !haveCompOps : true;

    if (haveCompOps && !system.has(CRE_DESCOMP))
        system.addAllCompOps();
    system.addAdditionalOps();

    const int nexact = forward ? sweepParams.get_forward_starting_size()
                               : sweepParams.get_backward_starting_size();
    if (!sweepParams.get_onedot() || dot_with_sys) {
        if (braquanta.size() != 0 && ketquanta.size() != 0)
            InitBlocks::InitNewSystemBlock(
                system, systemDot, newSystem, braState, ketState,
                sweepParams.get_sys_add(), dmrginp.direct(), integralIndex,
                DISTRIBUTED_STORAGE, haveNormOps, haveCompOps,
                NO_PARTICLE_SPIN_NUMBER_CONSTRAINT, braquanta, ketquanta);
        else {
            SpinQuantum moleculeQ = dmrginp.molecule_quantum();
            if ((dmrginp.calc_type() == RESPONSE ||
                 dmrginp.calc_type() == RESPONSELCC) &&
                system.get_sites()[0] != 0 &&
                system.get_sites()[0] >
                    dmrginp.num_occupied_orbitals()) { // response and reverse
                                                       // and after active sites
                dmrginp.set_molecule_quantum() =
                    SpinQuantum(2, SpinSpace(0), IrrepSpace(0));
                ...
                InitBlocks::InitNewSystemBlock(
                    ...
                    PARTICLE_NUMBER_CONSTRAINT);
                       ...
                       system.get_sites()[0] >
                           dmrginp
                               .num_occupied_orbitals()) { // response and
                                                           // reverse and after
                                                           // active sites
                dmrginp.set_molecule_quantum() =
                    SpinQuantum(1, SpinSpace(1), IrrepSpace(0));
                ...
                int totalN =
                    moleculeQ.particleNumber + moleculeQ.totalSpin.getirrep();
                int closedN = coreOrbs * 2;
                dmrginp.set_molecule_quantum() = SpinQuantum(
                    totalN - closedN + 1, SpinSpace(1), IrrepSpace(0));
                InitBlocks::InitNewSystemBlock(
                    system, systemDot, newSystem, braState, ketState,
                    sweepParams.get_sys_add(), dmrginp.direct(), integralIndex,
                    DISTRIBUTED_STORAGE, haveNormOps, haveCompOps,
    ...
    if (!dot_with_sys && sweepParams.get_onedot()) {
        if (braquanta.size() != 0 && ketquanta.size() != 0)
            InitBlocks::InitNewEnvironmentBlock(
                environment, systemDot, newEnvironment, system, systemDot,
                braState, ketState, sweepParams.get_sys_add(),
                sweepParams.get_env_add(), forward, dmrginp.direct(),
        ...
        if (braquanta.size() != 0 && ketquanta.size() != 0)
            ...
            if ((dmrginp.calc_type() == RESPONSE ||
                ...
                system.get_sites()[0] == 0 &&
                *system.get_sites().rbegin() >=
                    dmrginp.num_occupied_orbitals()) { // response and forward
                                                       // and after active sites
                dmrginp.set_molecule_quantum() =
                    SpinQuantum(2, SpinSpace(0), IrrepSpace(0));

                InitBlocks::InitNewEnvironmentBlock(
                    environment, environmentDot, newEnvironment, system,
                    systemDot, braState, ketState, sweepParams.get_sys_add(),
                    sweepParams.get_env_add(), forward, dmrginp.direct(),
                    sweepParams.get_onedot(), nexact, useSlater, integralIndex,
                    envnormops, envcompops, dot_with_sys,
                    PARTICLE_NUMBER_CONSTRAINT);
    ...
    // newSystem.set_loopblock(false); newEnvironment.set_loopblock(false);
    // environment.set_loopblock(false); newEnvironment.set_loopblock(false);
    if (dot_with_sys) {
    ...
    if (!dot_with_sys && sweepParams.get_onedot()) {
        if (braquanta.size() != 0 && ketquanta.size() != 0)
            InitBlocks::InitBigBlock(system, newEnvironment, big, braquanta,
                                     ketquanta);
        else
            InitBlocks::InitBigBlock(system, newEnvironment, big);
    } else {
        if (braquanta.size() != 0 && ketquanta.size() != 0)
            InitBlocks::InitBigBlock(newSystem, newEnvironment, big, braquanta,
                                     ketquanta);
        else
            InitBlocks::InitBigBlock(newSystem, newEnvironment, big);
    }
...
double SpinAdapted::Sweep::do_one(SweepParams &sweepParams, const bool &warmUp,
                                  const bool &forward, const bool &restart,
    ...
    if (dmrginp.get_sweep_type() == PARTIAL) {
        ...
        StackSpinBlock::restore(forward, sites, system,
                                sweepParams.current_root(),
                                sweepParams.current_root());
        system.set_twoInt(integralIndex);
    } else
        InitBlocks::InitStartingBlock(
            system, forward, sweepParams.current_root(),
            sweepParams.current_root(), sweepParams.get_forward_starting_size(),
            sweepParams.get_backward_starting_size(), restartSize, restart,
            warmUp, integralIndex);
                                     ...
                                     // blocking iterations needed in one sweep
    {
        ...
        if (dmrginp.no_transform() ||
            (sweepParams.get_sweep_iter() - sweepParams.get_restart_iter() ==
                 0 &&
...
void SpinAdapted::Sweep::Startup(SweepParams &sweepParams,
                                 StackSpinBlock &system,
                                 StackSpinBlock &newSystem) {
    mcheck("at the start of block and decimate");
    dmrginp.guessgenT->start(); // timer starts
    bool forward = (system.get_sites()[0] ==
                    0); // if first site is 0, then it's forward sweep
    ...
    if (forward) {
        systemDotStart = dmrginp.spinAdapted()
                             ? *system.get_sites().rbegin() + 1
                             : (*system.get_sites().rbegin()) / 2 + 1;
        systemDotEnd = systemDotStart + systemDotSize;
    } else {
        systemDotStart = dmrginp.spinAdapted()
                             ? system.get_sites()[0] - 1
                             : (system.get_sites()[0]) / 2 - 1;
        systemDotEnd = systemDotStart - systemDotSize;
    }
    ...
    systemDot =
        StackSpinBlock(systemDotStart, systemDotEnd, system.get_integralIndex(),
                       true); // default is_complement=false

    const int nexact = forward ? sweepParams.get_forward_starting_size()
                               : sweepParams.get_backward_starting_size();

    system.addAdditionalOps(); // communicate between different processors,
                               // broadcast operators from system block
    InitBlocks::InitNewSystemBlock(
        system, systemDot, newSystem, sweepParams.current_root(),
        sweepParams.current_root(), sweepParams.get_sys_add(), dmrginp.direct(),
    ...
    // DensityMatrix transformmatrix; // FIXME pay attention to this: density
    // matrix with certain quantum
    // transformmatrix.allocate(newSystem.get_stateInfo());
    StackDensityMatrix transformmatrix(newSystem.get_stateInfo());
    long requiredData = getRequiredMemory(newSystem.get_stateInfo(),
                                          transformmatrix.get_deltaQuantum());
    ...
    // if (mpigetrank() == 0) {
    double minval = 1e12;
    boost::shared_ptr<StackSparseMatrix> h =
        newSystem.get_op_array(HAM).get_element(0).at(0);
    ...
    if (mpigetrank() == 0) {
        ...
        int keptstates = sweepParams.get_keep_states() / 2,
            keptqstates = sweepParams.get_keep_states() - keptstates;
        int totalstatesbydm =
            min(static_cast<int>(inorderwts.size()), keptstates);
        int totalstatesbyquanta =
            min(static_cast<int>(inorderwts.size()), keptstates + keptqstates) -
            totalstatesbydm;
        if (totalstatesbyquanta < 0)
            totalstatesbyquanta = 0;

        p2out << "\t\t\t total states using dm and quanta " << totalstatesbydm
              << " " << totalstatesbyquanta << endl;

        double error = assign_matrix_by_dm(
            rotateMatrix, energies, transformmatrix, inorderwts, wtsbyquanta,
            totalstatesbydm, totalstatesbyquanta, newSystem.size(),
            2 * totalstatesbydm);
    ...
    p2out << dmrginp.guessgenT << " " << dmrginp.multiplierT << " "
          << dmrginp.operrotT << "  " << globaltimer.totalwalltime()
          << " timer " << endl;
    ...
    // p3out << dmrginp.otherrotation<<" "<<dmrginp.spinrotation<<"
    // "<<dmrginp.operrotT<<" rotations time "<<endl;
    p2out << "oneindexopmult   twoindexopmult   Hc  couplingcoeff" << endl;
    p2out << dmrginp.oneelecT << " " << dmrginp.twoelecT << " "
          << dmrginp.hmultiply << " " << dmrginp.couplingcoeff << " hmult"
          << endl;
    p2out << dmrginp.buildsumblock << " " << dmrginp.buildblockops
          << " build block" << endl;
