...
*/
void evaluateOverlapAndHamiltonian(int state1, int state2, double *o,
                                   double *h) {
...
void readMPSFromDiskAndInitializeStaticVariables(bool initializeDotBlocks) {
    ...
    if (dmrginp.spinAdapted() && dmrginp.add_noninteracting_orbs() &&
        ...
        newstartingBlock.BuildSumBlock(NO_PARTICLE_SPIN_NUMBER_CONSTRAINT, s,
                                       dummyblock);
        if (mpigetrank() == 0)
            MPS::siteBlocks.push_back(
                newstartingBlock); // alway make transpose operators as well
    } else {
        int i = 0;
        if (mpigetrank() == 0)
            MPS::siteBlocks.push_back(StackSpinBlock(
                i, i, 0, false)); // alway make transpose operators as well
    }
...
void RDM(char* infile)
{
  ...
  boost::shared_ptr<Npdm::Npdm_driver_base> npdm_driver =
boost::shared_ptr<Npdm::Npdm_driver_base>( new Npdm::Twopdm_driver(
dmrginp.last_site() ) );

  dmrginp.setimplicitTranspose() = false;
  dmrginp.setStateSpecific() = true;
  dmrginp.do_pdm() = true;
  dmrginp.oneindex_screen_tol() = 0.0; //need to turn screening off for one
index ops dmrginp.twoindex_screen_tol() = 0.0; //need to turn screening off for
two index ops
  ...
  for (int i=0; i<nstates; i++) {
    for (int j=i; j<i+1; j++) {
      ...
      sweepParams = sweep_copy; direction = direction_copy; restartsize =
restartsize_copy; if (mpigetrank() == 0) {
...
restartsize_copy;
      ...
      SweepGenblock::do_one(sweepParams, false, !direction, false, 0, states[i],
states[j]); //this will generate the cd operators dmrginp.set_fullrestart() =
false;


      //npdm_do_one_sweep(*npdm_driver, sweepParams, false, direction, false, 0,
states[i], states[j]); SweepTwopdm::do_one(sweepParams, false, direction, false,
0, states[i], states[j]);
    }
  }
...
void test(char *infile) {
    ...
    std::vector<std::vector<double>> ham(nstates,
                                         std::vector<double>(nstates, 0.0));
    std::vector<std::vector<double>> Overlap(nstates,
                                             std::vector<double>(nstates, 0.0));
...
/*
void evaluateOverlapAndHamiltonian1(unsigned long *occ, int length, double* o,
double* h) { MPS dmrgc(occ, length); dmrgc.writeToDiskForDMRG(2, false);
