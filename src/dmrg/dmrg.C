...
int calldmrg(char *input, char *output) {
    ...
    if (find(m_calc_procs.begin(), m_calc_procs.end(), rank) !=
        m_calc_procs.end()) {
        string &lprefix = dmrginp.load_prefix();
        string &sprefix = dmrginp.save_prefix();
        string oldrank = str(boost::format("%s%i") % "/node" % rank);
        string newrank =
            str(boost::format("%s%i") % "/node" %
                (find(m_calc_procs.begin(), m_calc_procs.end(), rank) -
                 m_calc_procs.begin()));
        int index = 0;
        index = lprefix.find(oldrank, index);
        lprefix.replace(index, oldrank.length(), newrank);
        int index2 = 0;
        index2 = sprefix.find(oldrank, index2);
    ...
    int oldset, newset;
    // if only a subset of processors are being used, then unset processor
    // affinity
    ...
    if (find(m_calc_procs.begin(), m_calc_procs.end(), rank) !=
        ...
        for (int i = 0; i < calc.size(); i++) {
            if (mpigetrank() == i) {
                cout << "processor: " << rank << "  numthrds: " << MAX_THRD
                     << "  node name: " << processor_name << endl;
            }
            calc.barrier();
        }
#endif
        ...
        pout << "**** STACK MEMORY REMAINING ***** "
             << 1.0 * (Stackmem[0].size - Stackmem[0].memused) *
                    sizeof(double) / 1.e9
             << " GB" << endl;
        ...
        if (dmrginp.calc_type() == COMPRESS) {
            ...
            int targetState, baseState, correctionVector, firstorderstate;
            {
                ...
                // initialize state info and canonicalize wavefunction is always
                // done using onedot algorithm
                if (mpigetrank() == 0) {
                    Sweep::InitializeStateInfo(sweepParams, direction,
                                               baseState);
                    Sweep::InitializeStateInfo(sweepParams, !direction,
                                               baseState);
                    Sweep::CanonicalizeWavefunction(sweepParams, direction,
                                                    baseState);
                    Sweep::CanonicalizeWavefunction(sweepParams, !direction,
                                                    baseState);
                    Sweep::CanonicalizeWavefunction(sweepParams, direction,
            ...
            // this genblock is required to generate all the nontranspose
            // operators
            dmrginp.setimplicitTranspose() = false;
            SweepGenblock::do_one(sweepParams, false, false, false, restartsize,
                                  baseState, baseState);
            ...
            // its product with V will have a larger bond dimension and is being
            // compressed it is called the target state
            ...
            if (mpigetrank() == 0 && !RESTART && !FULLRESTART &&
                ...
                for (int l = 0; l < dmrginp.projectorStates().size(); l++) {
                    Sweep::InitializeStateInfo(sweepParams, direction,
                                               dmrginp.projectorStates()[l]);
                    Sweep::InitializeStateInfo(sweepParams, !direction,
                                               dmrginp.projectorStates()[l]);
                    Sweep::CanonicalizeWavefunction(
                        sweepParams, direction, dmrginp.projectorStates()[l]);
                    Sweep::CanonicalizeWavefunction(
                        sweepParams, !direction, dmrginp.projectorStates()[l]);
                    Sweep::CanonicalizeWavefunction(
                        sweepParams, direction, dmrginp.projectorStates()[l]);
            ...
            if (dmrginp.get_sweep_type() == FULL)
                responseSweep(sweep_tol, dmrginp.targetState(),
                              dmrginp.projectorStates(), dmrginp.baseStates());
            else
                responsepartialSweep(sweep_tol, dmrginp.targetState(),
                                     dmrginp.projectorStates(),
                                     dmrginp.baseStates());

        } else if (dmrginp.calc_type() == RESPONSE ||
                   dmrginp.calc_type() == RESPONSELCC ||
                   dmrginp.calc_type() == RESPONSEAAAV ||
                   dmrginp.calc_type() == RESPONSEAAAC) {
            // compressing the V|\Psi_0>, here \Psi_0 is the basestate and
            // its product with V will have a larger bond dimension and is being
            // compressed it is called the target state
            // dmrginp.setimplicitTranspose() = false;
            ...
            if (mpigetrank() == 0) {
                ...
                for (int istate = 0; istate < dmrginp.nroots(); istate++)
                    for (int j = istate; j < dmrginp.nroots(); j++) {
                        int integralIndex = 0;
                        Sweep::InitializeOverlapSpinBlocks(
                            sweepParams, !direction, j, istate, integralIndex);
                        Sweep::InitializeOverlapSpinBlocks(
                            sweepParams, direction, j, istate, integralIndex);
                    }
            ...
            if (dmrginp.get_sweep_type() != FULL)
                ...
                if (RESTART && !FULLRESTART)
                    ...
                    if (!direction) {
                        double last_fe = Sweep::do_one(
                            sweepParams, false, direction, true, restartsize);
                    }
            ...
            Npdm::npdm(NPDM_THREEPDM, true);
        } else if (dmrginp.calc_type() == MPS_NEVPT ||
                   dmrginp.calc_type() == RESTART_MPS_NEVPT) {
            mps_nevpt::mps_nevpt(sweep_tol);
        } else {
            pout << "Invalid calculation types" << endl;
            abort();
        }
        /*
      case (TWOPDM):
      ...
      case(RESTART_NEVPT2):
        ...
        */
        ...
        pout << setprecision(3)
             << "\t\t\t BLOCK Wall Time (seconds): " << walltime << endl;
...
void dmrg(double sweep_tol) {
    ...
    if (!dmrginp.setStateSpecific()) {
        ...
        direction = false;
        while ((fabs(last_fe - old_fe) > sweep_tol) ||
               (fabs(last_be - old_be) > sweep_tol) ||
               (dmrginp.algorithm_method() == TWODOT_TO_ONEDOT &&
                dmrginp.twodot_to_onedot_iter() + 1 >=
            ...
            direction = true;
            pout << "\t\t\t Finished Sweep Iteration "
                 << sweepParams.get_sweep_iter() << endl;
        ...
        for (int i = 0; i < nroots; i++) {
            ...
            for (int j = 0; j < i; j++) {
                ...
                Sweep::InitializeOverlapSpinBlocks(sweepParams, direction, i, j,
                                                   integralIndex);
                Sweep::InitializeOverlapSpinBlocks(sweepParams, !direction, i,
                                                   j, integralIndex);
            }
            dmrginp.set_algorithm_method() = atype;
            ...
            dmrg_stateSpecific(sweep_tol, i);
            pout << "STATE SPECIFIC CALCULATION FOR STATE: " << i << " FINSIHED"
                 << endl;
        }
    ...
    if (!dmrginp.setStateSpecific()) {
        ...
        while ((fabs(last_fe - old_fe) > sweep_tol) ||
            ...
            if (dmrginp.max_iter() <= sweepParams.get_sweep_iter())
                break;
            last_be =
                Sweep::do_one_partial(sweepParams, false, direction, false, 0);
            direction = !direction;
            // last_be = Sweep::do_one_partial(sweepParams, false, true, false,
            // 0); direction = true;
            pout << "\t\t\t Finished Sweep Iteration "
                 << sweepParams.get_sweep_iter() << endl;
            ...
            last_fe =
                Sweep::do_one_partial(sweepParams, false, direction, false, 0);
            direction = !direction;
            // last_fe = Sweep::do_one_partial(sweepParams, false, false, false,
            // 0); direction = false;
...
void responseSweep(double sweep_tol, int targetState, vector<int> &projectors,
                   vector<int> &baseStates) {
    double last_fe = 1.e6;
    double last_be = 1.e6;
    double old_fe = 0.;
    double old_be = 0.;
    ...
    if (FULLRESTART) {
        ...
        last_fe = SweepResponse::do_one(sweepParams, warmUp, direction, restart,
                                        restartSize, targetState, projectors,
                                        baseStates);
        bool tempdirection;
    ...
    while (true) {
        ...
        last_be = SweepResponse::do_one(sweepParams, warmUp, !direction,
                                        restart, restartSize, targetState,
                                        projectors, baseStates);
        p1out << "\t\t\t Finished Sweep Iteration "
              << sweepParams.get_sweep_iter() << endl;
...
void responsepartialSweep(double sweep_tol, int targetState,
    ...
    if (FULLRESTART) {
        ...
        SpinQuantum oldq;
        // take the basestate wavefunction and expand it to accomodate large end
        // block
        if (mpigetrank() == 0) {
        ...
        if (dmrginp.calc_type() == RESPONSEAAAV) {
            ...
            StackSpinBlock system = StackSpinBlock::buildBigEdgeBlock(
                start, end, false, true, 1, false);
            ...
            newSystem.BuildSumBlock(NO_PARTICLE_SPIN_NUMBER_CONSTRAINT, system,
                                    site);
            ...
            if (mpigetrank() == 0) {
                ...
                w2.initialise(dmrginp.effective_molecule_quantum_vec(),
                              *state.leftStateInfo, newSystem.get_stateInfo(),
                              true);
                w2.UnCollectQuantaAlongColumns(*state.leftStateInfo,
                                               newSystem.get_stateInfo());
                for (int i = 0; i < w.nrows(); i++)
                    for (int j = 0; j < w.ncols(); j++)
                        if (w.allowed(i, j)) {
                            int J =
                                newSystem.get_stateInfo()
                                    .unCollectedStateInfo->quantaMap(0, j)[0];
                            copy(w(i, j), w2(i, J));
                        }
                w2.CollectQuantaAlongColumns(
                    *state.leftStateInfo,
                    const_cast<StateInfo &>(
                        *newSystem.get_stateInfo().unCollectedStateInfo));
                StateInfo bigstate;
                TensorProduct(
                    *state.leftStateInfo,
                    const_cast<StateInfo &>(newSystem.get_stateInfo()),
                    bigstate, PARTICLE_SPIN_NUMBER_CONSTRAINT);
            ...
            sites[0] = start - 1;
            sites[1] = end - 1;
            StackSpinBlock::store(false, sites, newSystem, targetState,
                                  baseStates[0]);
            // system.deallocate(); system.clear();

            newSystem.set_integralIndex() = 0;
            // system = StackSpinBlock::buildBigEdgeBlock(start, end, false,
            // true, 0, false);
            StackSpinBlock::store(false, sites, newSystem, targetState,
                                  baseStates[0]);
...

            {
                ...
                newSystem.set_integralIndex() = 0;
                newSystem.setstoragetype(DISTRIBUTED_STORAGE);
                newSystem.BuildSumBlock(NO_PARTICLE_SPIN_NUMBER_CONSTRAINT,
                                        system, site);
                StackSpinBlock::store(false, sites, newSystem, targetState,
                                      targetState);
                StackSpinBlock::store(false, sites, newSystem, baseStates[0],
                                      baseStates[0]);
                newSystem.deallocate();
                newSystem.clear();
                system.removeAdditionalOps();
            ...
            if (mpigetrank() == 0) {
                Sweep::InitializeStateInfoPartialSweep(sweepParams, true,
                                                       baseStates[0]);
                Sweep::CanonicalizeWavefunctionPartialSweep(sweepParams, false,
                                                            baseStates[0]);
                Sweep::CanonicalizeWavefunctionPartialSweep(sweepParams, true,
                                                            baseStates[0]);
                Sweep::CanonicalizeWavefunctionPartialSweep(sweepParams, false,
                                                            baseStates[0]);
            }
            ...
            InitBlocks::InitStartingBlock(
                forwardStart, true, targetState, targetState,
                sweepParams.get_forward_starting_size(),
                sweepParams.get_backward_starting_size(), 0, false, false, 0);
            sites[0] = 0;
            sites[1] = 0;
            StackSpinBlock::store(true, sites, forwardStart, targetState,
                                  targetState);

            forwardStart.deallocate();
            forwardStart.clear();
            InitBlocks::InitStartingBlock(
                forwardStart, true, baseStates[0], baseStates[0],
                sweepParams.get_forward_starting_size(),
                sweepParams.get_backward_starting_size(), 0, false, false, 0);
            sites[0] = 0;
            sites[1] = 0;
            StackSpinBlock::store(true, sites, forwardStart, baseStates[0],
                                  baseStates[0]);

            forwardStart.deallocate();
            forwardStart.clear();
            InitBlocks::InitStartingBlock(
                forwardStart, true, targetState, baseStates[0],
                sweepParams.get_forward_starting_size(),
                sweepParams.get_backward_starting_size(), 0, false, false, 1);
            sites[0] = 0;
            sites[1] = 0;
            StackSpinBlock::store(true, sites, forwardStart, targetState,
                                  baseStates[0]);
            ...
            if (mpigetrank() == 0)
                StackWavefunction::ChangeLastSite(dmrginp.last_site() - 1,
                                                  dmrginp.getPartialSweep() - 1,
                                                  baseStates[0]);
            ...
            if (mpigetrank() == 0) {
                std::vector<SpinQuantum> quanta =
                    newSystem.get_stateInfo().quanta;
                ...
                for (int i = 0; i < si.quanta.size(); i++) {
                    ...
                    for (int iSub = 0; iSub < oldToNewStateI.size(); iSub++) {
                        ...
                        int lindex = si.unCollectedStateInfo
                                         ->leftUnMapQuanta[unCollectedI];
                        int rindex = si.unCollectedStateInfo
                                         ->rightUnMapQuanta[unCollectedI];

                        if (lindex == lsi.quanta.size() -
                                          1) { // this is the last index so all
                                               // orbs are doubly occupied
                            if (rotation[i].Ncols() != 0) {
                                cout << "something wrong in writing the matrix"
                                     << endl;
                                abort();
                            } else {
                                rotation[i].ReSize(si.quantaStates[i], 1);
                                rotation[i] = 0.0;
                        ...
                        rowindex +=
                            si.unCollectedStateInfo->quantaStates[unCollectedI];
                    }
                }
                sites[0] = start - 1;
                sites[1] = dmrginp.last_site() - 1;
            ...
            sites[0] = start;
            sites[1] = end - 1;
            StackSpinBlock::store(false, sites, system, targetState,
                                  baseStates[0]);
            // newSystem.deallocate();
            ...
            system = StackSpinBlock::buildBigEdgeBlock(start, end, true, true,
                                                       0, true);
            StackSpinBlock::store(false, sites, system, targetState,
                                  targetState);
            StackSpinBlock::store(false, sites, system, baseStates[0],
                                  baseStates[0]);
            system.removeAdditionalOps();
            system.deallocate();
            system.clear();
            ...
            if (mpigetrank() == 0) {
                Sweep::InitializeStateInfoPartialSweep(sweepParams, false,
                                                       baseStates[0]);
                Sweep::CanonicalizeWavefunctionPartialSweep(sweepParams, true,
                                                            baseStates[0]);
                Sweep::CanonicalizeWavefunctionPartialSweep(sweepParams, false,
        ...
        last_be = SweepResponse::do_one(sweepParams, warmUp, direction, restart,
                                        restartSize, targetState, projectors,
                                        baseStates);
        dmrginp.set_algorithm_method() = atype;
        last_fe = SweepResponse::do_one(sweepParams, !warmUp, !direction,
...
void compress(double sweep_tol, int targetState, int baseState) {
    ...
    sweepParams.current_root() = -1;
    // this is the warmup sweep, the baseState is used as the initial guess for
    // the targetState
    last_fe = SweepCompress::do_one(sweepParams, true, true, false, 0,
    ...
    while (true) {
        ...
        if (dmrginp.max_iter() <= sweepParams.get_sweep_iter())
            break;
        last_be = SweepCompress::do_one(sweepParams, false, false, false, 0,
                                        targetState, baseState);
        direction = true;
        pout << "\t\t\t Finished Sweep Iteration "
             << sweepParams.get_sweep_iter() << endl;
        ...
        last_fe = SweepCompress::do_one(sweepParams, false, true, false, 0,
                                        targetState, baseState);
        direction = false;
    ...
    // one has to canonicalize the wavefunction with atleast 3 sweeps, this is a
    // quirk of the way we transform wavefunction
    algorithmTypes atype;
    atype = dmrginp.algorithm_method();
    dmrginp.set_algorithm_method() = ONEDOT;
    if (mpigetrank() == 0) {
...
void dmrg_stateSpecific(double sweep_tol, int targetState) {
    ...
    // initialize array of size m_maxiter or dmrginp.max_iter() for dw and
    // energy
    ...
    pout << "Converged Energy  " << sweepParams.get_lowest_energy()[0]
         << std::endl;
    if (dmrginp.max_iter() <= sweepParams.get_sweep_iter()) {
...
void Npdm(int pdm, bool restartpdm, bool transitionpdm) {
    ...
    // Screening can break things for NPDM (e.g. smaller operators won't be
    // available from which to build larger ones etc...?)
    dmrginp.oneindex_screen_tol() =
        0.0; // need to turn screening off for one index ops
    dmrginp.twoindex_screen_tol() =
        0.0; // need to turn screening off for two index ops
...

    {
        ...
        for (int state = 0; state < dmrginp.nroots(); state++) {
            ...
            SweepGenblock::do_one(sweepParams, false, !direction, false, 0,
                                  state,
                                  state); // this will generate the cd operators
            if (pdm == 1)
                SweepOnepdm::do_one(sweepParams, false, direction, false, 0,
                                    state);
            // if (pdm == 2) SweepTwopdm::do_one(sweepParams, false, direction,
            // false, 0, state); else if (npdm_order == NPDM_TWOPDM)
...
/*
void restartResponseSweep(double sweep_tol, int targetState, int
correctionVector, int baseState)
{
  double last_fe = 10.e6;
  double last_be = 10.e6;
  double old_fe = 0.;
  ...
  last_fe = SweepResponse::do_one(sweepParams, warmUp, direction, restart,
restartSize, targetState, correctionVector, baseState);
  ...
  while ( true)
    {
      ...
      if(dmrginp.max_iter() <= sweepParams.get_sweep_iter())
        break;
      last_be = SweepResponse::do_one(sweepParams, warmUp, !direction, restart,
restartSize, targetState, correctionVector, baseState); p1out << "\t\t\t
Finished Sweep Iteration "<<sweepParams.get_sweep_iter()<<endl;
