...
void SpinAdapted::Input::ReadMeaningfulLine(istream &input, string &msg,
                                            int msgsize) {
    bool readmore = true;
    while (readmore && !input.eof()) {
        ...
        msg = string(msgctr);
        if (msg.size() == msgsize) {
            pout << "in the process of reading line beginning with " << endl
                 << msg << endl;
...
SpinAdapted::Input::Input(const string &config_name, const std::string &contents) {
    ...
    if (rank == 0) {
        ...
        unique_ptr<istream> input_p;
        if (config_name != "")
            input_p = unique_ptr<ifstream>(new ifstream(config_name.c_str()));
        else
            input_p = unique_ptr<stringstream>(new stringstream(contents));

        istream &input = *input_p;
        ...
        while (msg.size() != 0) {
            ...
            if (boost::iequals(keyword, "orbs") ||
                boost::iequals(keyword, "orbitals") ||
                boost::iequals(keyword, " orbitals")) {
                if (usedkey[ORBS] == 0)
                    usedkey_error(keyword, msg);
                usedkey[ORBS] = 0;
                if (tok.size() < 2) {
                    pout << "keyword orbs should be followed by atleast a "
                            "single filename and then an end line"
                         << endl;
                ...
                usedkey[MAXM] = 0;
                if (tok.size() != 2) {
                    pout << "keyword maxM should be followed by a single "
                            "number and then an end line"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                if (tok.size() != 1) {
                    pout << "keyword nonspinadated is a stand alone keyword"
                         << endl;
                    pout << msg << endl;
                    abort();
                }
                ...
                if (tok.size() != 1) {
                    pout << "keyword bogoliubov is a stand alone keyword"
                         << endl;
                    pout << msg << endl;
                    abort();
                }
                ...
                usedkey[WARMUP] = 0;
                if (tok.size() != 2) {
                    pout << "must specify warmup type with keyword warmup"
                         << endl;
                    pout << msg << endl;
                    abort();
                }
                if (boost::iequals(tok[1], "wilson")) {
                    m_warmup = WILSON; // default option select the lowest
                                       // energy slater determinants
                ...
                usedkey[STARTM] = 0;
                if (tok.size() != 2) {
                    pout << "keyword startM should be followed by a single "
                            "number and then an end line"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                pout << "------------------------------------------------------"
                        "--------------"
                     << endl;
                pout << "WARNING: THIS OPTION IMPLIES THAT A PREVIOUS DMRG "
                        "CALCULATION HAS ALREADY BEEN PERFORMED"
                     << endl;
                pout << "THIS CALCULATION WILL TAKE THE PREVIOUS WAVEFUNCTIONS "
                        "AND REFINE THEM"
                     << endl;
                pout << "------------------------------------------------------"
                        "--------------"
                ...
                pout << "------------------------------------------------------"
                        "--------------"
                     << endl;
                pout << "DONT USE THIS OPTION BECAUSE IT SLOWS DOWN THE CODE."
                     << endl;
                pout << "------------------------------------------------------"
                        "--------------"
                     << endl;
                ...
                if (tok.size() != 2) {
                    pout << "keyword occ should be followed by a single number "
                ...
                usedkey[LASTM] = 0;
                if (tok.size() != 2) {
                    pout << "keyword lastM should be followed by a single "
                            "number and then an end line"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                if (tok.size() < 2) {
                    pout << "calcprocs should be followed by a list of "
                            "processor ids"
                         << endl;
                }
...
#endif
                        pout << "calcprocessor number is greater than total "
                ...
                usedkey[REORDER] = 0;
                if (tok.size() != 2) {
                    pout << "keyword reorder should be followed by the "
                            "filename and then an end line"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                if (tok.size() != 2) {
                    perr << "keyword gaopt should be followed by the filename "
                            "and then an end line"
                         << endl;
                ...
                if (tok.size() != 1) {
                    perr << "keyword fiedler should not be followed by anything"
                         << endl;
                    perr << "error found in the following line " << endl;
                    perr << msg << endl;
                    abort();
                ...
                m_reorderType = FIEDLER;
            } else if (boost::iequals(keyword, "noreorder") ||
                       boost::iequals(keyword, "nofiedler")) {
                ...
                if (tok.size() != 2 && tok.size() != 1) {
                    pout
                        << "keyword schedule should be followed by the keyword "
                           "\"default\" and then an end line or just an endline"
                        << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                boost::split(schd_tok, msg, is_any_of(" \t"),
                             token_compress_on);
                if (tok.size() != 1) {
                ...
                while (!boost::iequals(schd_tok[0], "END")) {
                    ...
                    if (schd_tok.size() != 4) {
                        pout << "Each line of the schedule contain four "
                                "entries sweep_iteration   #retained states   "
                                "Davidson tolerance     noise"
                             << endl;
                    ...
                    if (m_sweep_state_schedule[i] <= 0) {
                        pout
                            << "Number of retained states cannot be less than 0"
                            << endl;
                        pout << "error found in the following line " << endl;
                        pout << msg << endl;
                    }
                    if (i > 0 && m_sweep_iter_schedule[i] <=
                                     m_sweep_iter_schedule[i - 1]) {
                        pout << "Sweep iteration at a given line should be "
                                "higher than the previous sweep iteration"
                             << endl;
                        pout << "this sweep iteration "
                             << m_sweep_iter_schedule[i] << endl;
                        pout << "previous sweep iteration "
                             << m_sweep_iter_schedule[i - 1] << endl;
                        pout << "error found in the following line " << endl;
                        pout << msg << endl;
                        abort();
                    }
            ...
            else if (boost::iequals(keyword, "sym") ||
                     boost::iequals(keyword, "symmetry") ||
                     boost::iequals(keyword, "point_group") ||
                     boost::iequals(keyword, "pg")) {
                if (usedkey[SYM] == 0)
                    usedkey_error(keyword, msg);
                usedkey[SYM] = 0;
                ...
                // string sym;
                if (tok.size() != 2) {
                    pout << "keyword sym should be followed by a single string "
                            "and then an end line"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
            ...
            else if (boost::iequals(keyword, "mkl_thrds") ||
                     boost::iequals(keyword, "threads_mkl"))
                m_mkl_thrds = atoi(tok[1].c_str());
            else if (boost::iequals(keyword, "quanta_thrds") ||
                     boost::iequals(keyword, "threads_quanta"))
                m_quanta_thrds = atoi(tok[1].c_str());
            else if (boost::iequals(keyword, "partialsweep")) {
                if (tok.size() != 2) {
                    pout << "keyword partialSweep should be followed by a "
                            "single integer!"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                if (tok.size() != 2) {
                    pout << "keyword num_thrds should be followed by a single "
                            "integer!"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                m_thrds_per_node.resize(m_calc_procs.size(),
                                        atoi(tok[1].c_str()));
            } else if (boost::iequals(keyword, "memory") ||
                       boost::iequals(keyword, "mem")) {
                if (tok.size() != 3) {
                    pout << "keyword should be followed by a number (memory "
                            "size) and either k, m or g"
                         << endl;
                    pout << "error found in the following line" << endl;
                    pout << msg << endl;
                    abort();
                ...
                if (boost::iequals(tok[2], "k"))
                    ...
                    pout << "the units of memory should be either: k, m, g"
                         << endl;
                    abort();
                }
            } else if (boost::iequals(keyword, "nelecs") ||
                       boost::iequals(keyword, "nelec")) {
                if (usedkey[NELECS] == 0)
                    usedkey_error(keyword, msg);
                usedkey[NELECS] = 0;
                if (tok.size() != 2) {
                    pout << "keyword nelec should be followed by a single "
                            "number and then an end line"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                if (n_elec < 0) {
                    pout << "Number of electrons cannot be less than 0."
                         << endl;
                    pout << "See the manual for further details." << endl;
                    abort();
                }
            } else if (boost::iequals(keyword, "nspin") ||
                       boost::iequals(keyword, "spin")) {
                if (usedkey[SPIN] == 0)
                    usedkey_error(keyword, msg);
                usedkey[SPIN] = 0;
                if (tok.size() != 2) {
                    pout << "keyword spin should be followed by a single "
                            "number and then an end line"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                if (n_spin < 0) {
                    pout << "Spin of the wavefunction cannot be less than 0."
                         << endl;
                    pout << "See the manual for further details." << endl;
                    abort();
                }
            } else if (boost::iequals(keyword, "nirrep") ||
                       boost::iequals(keyword, "irrep")) {
                if (usedkey[IRREP] == 0)
                    usedkey_error(keyword, msg);
                usedkey[IRREP] = 0;
                // When there are 2 irrep number, it means that calcultions of
                // transition density matrix between wavefunctions with
                // different irrep.
                if (tok.size() == 2)
                    m_total_symmetry_number =
                        IrrepSpace(atoi(tok[1].c_str()) - 1);
                else if (tok.size() == 3) {
                    m_bra_symmetry_number =
                        IrrepSpace(atoi(tok[1].c_str()) - 1);
                    m_total_symmetry_number =
                        IrrepSpace(atoi(tok[2].c_str()) - 1);
                    m_transition_diff_spatial_irrep = true;
                } else {
                    pout << "keyword irrep should be followed by one or two "
                            "numbers and then an end line"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                if (tok.size() != 1) {
                    pout << "The keyword response should not be followed by "
                            "anything!"
                         << endl;
                    abort();
                }
                ...
                if (tok.size() != 2) {
                    pout << "keyword compression should be followed by a "
                            "single number and then an end line"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                if (tok.size() != 4) {
                    pout << "keyword mps_nevpt should be followed by three "
                            "numbers and then an end line"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                //          vpt_1[static_cast<OnePerturbType>(i)]=
                //          OneElectronArray();
                //        }
                for (int i = 0; i < TwoPertEnd; i++) {
                    // vpt_2[static_cast<TwoPerturbType>(i)]=
                    // TwoElectronArray(TwoElectronArray::restrictedNonPermSymm);
                    vpt_2[static_cast<TwoPerturbType>(i)] =
                        PerturbTwoElectronArray();
                    // vpt_2[i].rhf = true;
                }
                ...
                if (tok.size() != 2) {
                    pout << "keyword nevpt_state_num should be followed by a "
                            "single integer than then an end line."
                         << endl;
                    abort();
                }
                ...
                if (tok.size() != 2) {
                    pout << "keyword maxj should be followed by a single "
                            "integer and then an end line."
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                m_calc_type = FCI;
                     ...
                     boost::iequals(keyword, "onerdm") ||
                     boost::iequals(keyword, "ordm"))
                m_calc_type = ONEPDM;
            else if (boost::iequals(keyword, "twopdm") ||
                     boost::iequals(keyword, "twordm") ||
                     boost::iequals(keyword, "trdm"))
                m_calc_type = TWOPDM;
            ...
            else if (boost::iequals(keyword, "restart_onepdm") ||
                     boost::iequals(keyword, "restart_onerdm") ||
                     boost::iequals(keyword, "restart_ordm"))
                m_calc_type = RESTART_ONEPDM;
            else if (boost::iequals(keyword, "restart_twopdm") ||
                     boost::iequals(keyword, "restart_twordm") ||
                     boost::iequals(keyword, "restart_trdm"))
                m_calc_type = RESTART_TWOPDM;
            else if (boost::iequals(keyword, "restart_threepdm") ||
                     boost::iequals(keyword, "restart_threerdm"))
                m_calc_type = RESTART_THREEPDM;
            else if (boost::iequals(keyword, "restart_fourpdm") ||
                     boost::iequals(keyword, "restart_fourrdm"))
                m_calc_type = RESTART_FOURPDM;
            else if (boost::iequals(keyword, "restart_nevpt2_npdm"))
                m_calc_type = RESTART_NEVPT2PDM;
            else if (boost::iequals(keyword, "transition_onepdm") ||
                     boost::iequals(keyword, "transition_onerdm") ||
                     boost::iequals(keyword, "tran_onepdm"))
                m_calc_type = TRANSITION_ONEPDM;
            else if (boost::iequals(keyword, "transition_twopdm") ||
                     boost::iequals(keyword, "transition_twordm") ||
                     boost::iequals(keyword, "tran_twopdm"))
                m_calc_type = TRANSITION_TWOPDM;
            else if (boost::iequals(keyword, "transition_threepdm") ||
                     boost::iequals(keyword, "transition_threerdm") ||
                     boost::iequals(keyword, "tran_threepdm"))
                m_calc_type = TRANSITION_THREEPDM;
            else if (boost::iequals(keyword, "restart_tran_onepdm") ||
                     boost::iequals(keyword, "restart_tran_onerdm") ||
                     boost::iequals(keyword, "restart_tran_ordm"))
                m_calc_type = RESTART_T_ONEPDM;
            else if (boost::iequals(keyword, "restart_tran_twopdm") ||
                     boost::iequals(keyword, "restart_tran_twordm") ||
                     boost::iequals(keyword, "restart_tran_trdm"))
                m_calc_type = RESTART_T_TWOPDM;
            else if (boost::iequals(keyword, "restart_tran_threepdm") ||
                     boost::iequals(keyword, "restart_tran_threerdm") ||
                     boost::iequals(keyword, "restart_tran_threerdm"))
                m_calc_type = RESTART_T_THREEPDM;
            else if (boost::iequals(keyword, "nevpt2") ||
                     boost::iequals(keyword, "pt2")) {
                m_calc_type = NEVPT2;
                m_nevpt2 = true;
                ...
                m_nevpt2 = false;
                m_transition_diff_spatial_irrep = false;
            } else if (boost::iequals(keyword, "restart_nevpt2") ||
                       boost::iequals(keyword, "restart_pt2")) {
                ...
                m_nevpt2 = false;
                m_transition_diff_spatial_irrep = false;
            } else if (boost::iequals(keyword, "calc_ri4pdm") ||
                       boost::iequals(keyword, "ri4pdm")) {
                m_calc_ri_4pdm = true;
                ...
                if (tok.size() != 2) {
                    pout << "keyword M_nevpt2 should be followed by a single "
                            "float and then an end line."
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
            ...
            else if (boost::iequals(keyword, "prefix") ||
                     boost::iequals(keyword, "scratch")) {
                if (usedkey[PREFIX] == 0)
                    usedkey_error(keyword, msg);
                usedkey[PREFIX] = 0;
                m_load_prefix = tok[1];
            ...
            // user-defined initial HF wave function occupancies, in spin
            // orbital
                ...
                // the occupancies start from the second element of the tok
                // string
                // ++it;
                ...
                if (tok.size() != 2) {
                    pout << "keyword nroots should be followed by a single "
                            "integer and then an end line."
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                boost::split(weighttoken, msg, is_any_of(" \t"),
                             token_compress_on);
                ...
                if (weighttoken.size() != m_nroots + 1) {
                    pout << "keyword weights should be followed by floating "
                            "point numbers providing weights for "
                         << m_nroots << " states." << endl;
                    pout << "You could chose to omit the keyword weights in "
                            "which case the weights will be distributed "
                            "uniformly between the different roots"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                for (int i = 1; i < weighttoken.size(); i++) {
                    ...
                    if (m_weights[i - 1] < 1e-10) {
                        pout << "Weight of a state cannot be less than 1e.0e-10"
                             << endl;
                        pout << "error found in the following line " << endl;
                        pout << msg << endl;
                        abort();
                ...
                if (norm <= 1.e-10) {
                    pout << "Weights should add up to approximately 1.0. "
                            "Currently they add up to "
                         << norm << endl;
                    abort();
                }
            ...
            else if (boost::iequals(keyword, "docd") ||
                     boost::iequals(keyword, "do_npdm_ops")) {
                m_do_npdm_ops = true;
                ...
                if (tok.size() == 2)
                    ...
                    pout << "keyword " << keyword
                         << " should be followed by one or two numbers and "
                            "then an endline";
                    abort();
                }
            }

            else if (boost::iequals(keyword, "deflation_max_size") ||
                     boost::iequals(keyword, "max_deflation_size")) {
                if (tok.size() != 2) {
                    pout << "keyword " << keyword
                         << " should be followed by a single number and then "
                            "an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                if (tok.size() < 2) {
                    pout << "keyword projectorstates should be followed by "
                            "atleast a single number and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                if (tok.size() < 2) {
                    pout << "keyword basestate should be followed by a single "
                            "number and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                usedkey[MAXITER] = 0;
                if (tok.size() != 2) {
                    pout << "keyword maxiter should be followed by a single "
                            "integer and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
            ...
            else if (boost::iequals(keyword, "screen_tol") ||
                     boost::iequals(keyword, "screen_tolerance") ||
                     boost::iequals(keyword, "screening_tol") ||
                     boost::iequals(keyword, "screening_tolerance")) {
                if (usedkey[SCREEN_TOL] == 0)
                    usedkey_error(keyword, msg);
                usedkey[SCREEN_TOL] = 0;
                if (tok.size() != 2) {
                    pout << "keyword screen_tol should be followed by a single "
                            "number and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                if (tok.size() != 2) {
                    pout << "keyword oneindex_screen_tol should be followed by "
                            "a single number and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                if (tok.size() != 2) {
                    pout << "keyword twoindex_screen_tol should be followed by "
                            "a single number and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                if (tok.size() != 2) {
                    pout << "keyword twodot_to_onedot should be followed by a "
                            "single number and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
                ...
                if (tok.size() != 3) {
                    pout << "keyword twodot_noise should be followed by two "
                            "single numbers and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
            ...
            else if (boost::iequals(keyword, "sweep_tol") ||
                     boost::iequals(keyword, "sweep_tolerance")) {
                if (usedkey[SWEEP_TOL] == 0)
                    usedkey_error(keyword, msg);
                usedkey[SWEEP_TOL] = 0;
                if (tok.size() != 2) {
                    pout << "keyword sweep_tol should be followed by a single "
                            "number and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                ...
                if (m_sweep_tol < 1.0e-12) {
                    pout << "Sweep tolerance of less than 1.0e-12 may cause "
                            "convergence problems. Changing it to from "
                         << m_sweep_tol << " to 1.0e-12 instead." << endl;
                    m_sweep_tol = 1.0e-12;
                }
            }
                ...
                if (tok.size() != 2) {
                    pout << "keyword outputlevel should be followed by a "
                            "single integer and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
                    abort();
            ...
            else if (boost::iequals(keyword, "reset_iterations") ||
                     boost::iequals(keyword, "reset_iter") ||
                     boost::iequals(keyword, "reset_iters")) {
                m_reset_iterations = true;
            } else if (boost::iequals(keyword, "bra_M")) {
                // The number of renormalized bases in bra wave function.
                // Currently only used in transition denstiy matrix
                // calcualtions.
                if (tok.size() != 2) {
                    pout << "keyword bra_M should be followed by a single "
                            "integer and then an endline"
                         << endl;
                    pout << "error found in the following line " << endl;
                    pout << msg << endl;
        ...
        if (m_hf_occ_user == "") {
            pout << "need to specify hf_occ. The recommended default setting "
                    "is: hf_occ integral"
                 << endl;
            abort();
        }
        ...
        if (n_elec == -1) {
            if (!m_Bogoliubov) {
                pout << "number of electrons has to be specified using the "
                        "keyword nelec"
                     << endl;
                abort();
            } else {
        ...
        if (n_spin == -1) {
            pout << "spin of the wavefunction has to be specified using the "
                    "keyword spin"
                 << endl;
            abort();
        }
        ...
        if (sym == "trans" || sym == "lzsym")
            m_total_symmetry_number = IrrepSpace(
                m_total_symmetry_number.getirrep() +
                1); // in translational symmetry lowest irrep is 0 and not 1

        if (n_twodot_noise == 1) {
            if (m_algorithm_type == ONEDOT ||
                fabs(m_twodot_noise * m_twodot_gamma) <= NUMERICAL_ZERO) {
        ...
        if (m_calc_type == RESPONSELCC || m_calc_type == RESPONSEAAAV ||
            ...
            if (m_num_Integrals != 1) {
                pout << "LCC calculations should be performed with only one "
                        "integral file."
                     << endl;
                exit(0);
            }
            ...
            if (m_openorbs.size() == 0 && m_closedorbs.size() == 0) {
                pout << "LCC calculations should be performed with open and "
                        "closed orbitals specified."
                     << endl;
                exit(0);
            }
        }
    ...
    m_load_prefix = str(boost::format("%s%s%d%s") % m_load_prefix % "/node" %
                        mpigetrank() % "/");
    m_save_prefix = m_load_prefix;
    ...
    if ((m_calc_type == RESPONSE || m_calc_type == RESPONSEBW) &&
        m_num_Integrals != m_baseState.size() + 1) {
        pout << "number of integrals should be 1 more than the number of base "
                "states"
             << endl;
    ...
    if (m_Bogoliubov && m_num_Integrals > 1) {
        pout << "Currently the response code does not work with non-particle "
    ...
    for (int integral = 0; integral < m_num_Integrals; integral++) {
        ...
        if (m_Bogoliubov) {
            ...
            v_cccc[integral].rhf = true;
            v_cccd[integral].rhf = true;
            readorbitalsfile(orbitalfile[integral], v_1[integral],
                             v_2[integral], coreEnergy[integral],
                             v_cc[integral], v_cccc[integral], v_cccd[integral],
                             integral);
            assert(!m_add_noninteracting_orbs);
        } else if (m_calc_type == MPS_NEVPT ||
                   m_calc_type == RESTART_MPS_NEVPT) {
            readorbitalsfile(orbitalfile[integral], v_1[0], v_2[0], vpt_1,
                             vpt_2, coreEnergy[0]);
        } else {
            if (m_calc_type == RESPONSELCC || m_calc_type == RESPONSEAAAV ||
                m_calc_type == RESPONSEAAAC) {
                if (integral == 0) {
                    ...
                    for (int orb = 0; orb < m_norbs / 2; orb++) {
                        if (std::find(m_openorbs.begin(), m_openorbs.end(),
                                      orb) == m_openorbs.end() &&
                            std::find(m_closedorbs.begin(), m_closedorbs.end(),
                                      orb) == m_closedorbs.end())
                            m_activeorbs.push_back(orb);

                        if (std::find(m_openorbs.begin(), m_openorbs.end(),
                                      orb) != m_openorbs.end())
                            m_excitation[reorder.at(orb)] = 12;
                        else if (std::find(m_closedorbs.begin(),
                                           m_closedorbs.end(),
                                           orb) != m_closedorbs.end())
                            m_excitation[reorder.at(orb)] = 1;
                        else if (std::find(m_activeorbs.begin(),
                                           m_activeorbs.end(),
                                           orb) != m_activeorbs.end())
                            m_excitation[reorder.at(orb)] = 6;
                    }
                }
            } else
                readorbitalsfile(orbitalfile[integral], v_1[integral],
                                 v_2[integral], coreEnergy[integral], integral);
        }
    }

    m_molecule_quantum = SpinQuantum(
        m_alpha + m_beta, SpinSpace(m_alpha - m_beta), m_total_symmetry_number);
    ...
    if (rank == 0) {
        ...
        if (n_twodot_noise == 1) {
            pout << "\nScheduled random noise is disabled using twodot_noise \n"
                 << endl;
            fill(m_sweep_noise_schedule.begin(), m_sweep_noise_schedule.end(),
                 0.0);
        }
    ...
    //    cout << fixed << setprecision(12) << v_1[0](i,j) << " " << i << " " <<
    //    j << endl;
    ...
    // for (int i = 0; i < m_norbs; ++i)
    //  for (int j = 0; j < m_norbs; ++j)
    //    cout << fixed << setprecision(12) << v_cc[0](i,j) << " " << i << " "
    //    << j << endl;
...
void SpinAdapted::Input::readreorderfile(ifstream &dumpFile,
    ...
    while (msg.size() != 0) {
        for (int i = 0; i < tok.size(); i++)
            if (tok[i].size() != 0) {
                if (find(oldtonew.begin(), oldtonew.end(),
                         atoi(tok[i].c_str()) - 1) != oldtonew.end()) {
                    pout << "Orbital " << atoi(tok[i].c_str())
                         << " appears twice in reorder file" << endl;
                    pout << "error found at the following line " << endl;
                    pout << msg << endl;
                    abort();
                }
                // reorder is starting from 1 to n, but internally we store it
                // from 0 to n-1
                oldtonew.push_back(atoi(tok[i].c_str()) - 1);
                if (oldtonew.back() > m_norbs || oldtonew.back() < 0) {
                    pout << "Illegal orbital index " << atoi(tok[i].c_str())
                         << " in reorder file" << endl;
                    abort();
                }
            }
        msg.resize(0);
    ...
    if (oldtonew.size() != m_norbs / 2) {
        pout << "Numbers or orbitals in reorder file should be equal to "
             << m_norbs / 2 << " instead " << oldtonew.size() << " found "
             << endl;
        abort();
    }
}

void SpinAdapted::Input::readorbitalsfile(string &orbitalfile,
                                          OneElectronArray &v1,
                                          TwoElectronArray &v2,
                                          double &coreEnergy,
                                          int integralIndex) {
    ...
    if (rank == 0) {
        ...
        sprintf(ReorderFileName, "%s%s", save_prefix().c_str(),
                "/RestartReorder.dat");
    }
    ...
    if (integralIndex == 0) {
        if (get_restart() || get_fullrestart() || m_calc_type == COMPRESS ||
            m_calc_type == RESPONSE || m_calc_type == RESPONSEBW) {
            if (rank == 0) {
                ...
                if (!boost::filesystem::exists(ReorderFilePath)) {
                    pout << "===============" << endl;
                    pout << "This is a restart job and the reorder file "
                         << ReorderFileName << " should be present" << endl;
                    abort();
                } else {
                    pout << "================" << endl;
                    pout << "The Fiedler routine for finding the orbital "
                            "ordering has already been run."
                         << endl;
                    pout << "Using the reorder file " << ReorderFileName
                         << endl;
                    pout << "================" << endl;
            ...
            // read the reorder file or calculate the reordering using one of
            // the many options
    ...
    // the name reorder is confusing because it clases the with the m_reorder
    // member of the input class and these two are inverse of each other. the
    // reorder here helps one to go from the unreordered matrices to the
    // reordered matrices and the m_reorder member of input helps one to go in
    // the opposite direction e.g. the user defined orbital order (m_reorder)
    // could be
    // 2 3 1 4   which implies that O_reorder(1,2) -> O_unreordered(2,3)  (the
    // former is stored internally and the latter is what is given during input)
    // but for this m_reorder the reorder vector below would be 3 1 2 4 and
    // O_unreordered(1,2) -> O_reorder(3, 1)
    bool RHF = true;
    ...
    if (rank == 0) {
        ...
        while (!(boost::iequals(tok[0], "ISYM") ||
                 boost::iequals(tok[0], "&END"))) {
            for (int i = 0; i < tok.size(); i++) {
                if (boost::iequals(tok[i], "ORBSYM") || tok[i].size() == 0)
                    continue;
                ...
                if (sym == "trans")
                    ir += 1; // for translational symmetry the lowest irrep is 0
                if (sym == "lzsym")
                    ir = atoi(tok[i].c_str());
    ...
    if (m_calc_type != RESPONSEAAAV) {
        ...
        if (RHF) {
            oneIntegralMem = (m_norbs / 2 * (m_norbs / 2 + 1)) / 2;
            twoIntegralMem = twoedim * a * (a + 1) / 2 +
                             ((v + a) * a * ((v + a) * a + 1) / 2);
        } else {
            oneIntegralMem = m_norbs / 2 * (m_norbs / 2 + 1);
            twoIntegralMem =
                twoedim * a * (a + 1) / 2 + (v + a) * a * ((v + a) * a + 1) / 2;
        }
    }
    ...
    if (m_useSharedMemory) {
        if (integralIndex == 0) {
            segment.truncate((oneIntegralMem + twoIntegralMem) *
                             m_num_Integrals * sizeof(double));
            region = boost::interprocess::mapped_region{
                segment, boost::interprocess::read_write};
            memset(region.get_address(), 0.,
                   (oneIntegralMem + twoIntegralMem) * m_num_Integrals *
                       sizeof(double));
        }
#ifndef SERIAL
        ...
        v1.set_data() = static_cast<double *>(region.get_address()) +
                        (oneIntegralMem + twoIntegralMem) * integralIndex;
        v2.set_data() = static_cast<double *>(region.get_address()) +
                        oneIntegralMem +
                        (oneIntegralMem + twoIntegralMem) * integralIndex;
    } else {
        long integralMemorySize =
            (oneIntegralMem + twoIntegralMem) * m_num_Integrals;
        m_IntegralMemoryStart = Stackmem[0].allocate(integralMemorySize);
        v1.set_data() = (m_IntegralMemoryStart) +
                        (oneIntegralMem + twoIntegralMem) * integralIndex;
        v2.set_data() = (m_IntegralMemoryStart) + oneIntegralMem +
                        (oneIntegralMem + twoIntegralMem) * integralIndex;
    }
    if (rank == 0) {
        msg.resize(0);
        ReadMeaningfulLine(dumpFile, msg,
                           msgsize); // this if the first line with integrals
        ...
        while (msg.size() != 0) {
            ...
            i = atoi(tok[1].c_str()) - offset;
            j = atoi(tok[2].c_str()) - offset;
            k = atoi(tok[3].c_str()) - offset;
            l = atoi(tok[4].c_str()) - offset;
            ...
            if (i == -1 && j == -1 && k == -1 && l == -1) {
                ...
                v1(2 * reorder.at(i) + AOrbOffset,
                   2 * reorder.at(j) + AOrbOffset) = value;
                v1(2 * reorder.at(j) + AOrbOffset,
                   2 * reorder.at(i) + AOrbOffset) = value;
            } else {
                int I = reorder.at(i);
                int J = reorder.at(j);
                int K = reorder.at(k);
                int L = reorder.at(l);
                v2(2 * I + BOrbOffset, 2 * K + AOrbOffset, 2 * J + BOrbOffset,
                   2 * L + AOrbOffset) = value;
            }
            msg.resize(0);
            ReadMeaningfulLine(
                dumpFile, msg,
        ...
        if (m_norbs / 2 >= m_integral_disk_storage_thresh) //
        {
            for (int i = 0; i < m_spatial_to_spin.size() - 1; i++) {
                std::vector<int> orb;
                for (int j = m_spatial_to_spin[i]; j < m_spatial_to_spin[i + 1];
                     j += 2) {
                    orb.push_back(j / 2);
                }
    ...
    for (int i = 0; i < maxIter; i++) {
        MPI::COMM_WORLD.Bcast(v1.set_data() + i * maxint, maxint, MPI_DOUBLE,
                              0);
        MPI::COMM_WORLD.Barrier();
    }
    MPI::COMM_WORLD.Bcast(v1.set_data() + (maxIter)*maxint,
                          oneIntegralMem + twoIntegralMem - maxIter * maxint,
                          MPI_DOUBLE, 0);
    MPI::COMM_WORLD.Barrier();
...
void SpinAdapted::Input::readorbitalsfile(string &orbitalfile,
                                          OneElectronArray &v1,
                                          TwoElectronArray &v2,
                                          double &coreEnergy, PairArray &vcc,
                                          CCCCArray &vcccc, CCCDArray &vcccd,
                                          int integralIndex) {
    ...
    long oneIntegralMem, vccIntegralMem, twoIntegralMem, vccccIntegralMem,
        vcccdIntegralMem;
    long twoedim = v2.matDim, vccccdim = vcccc.matDim, vcccddim = vcccd.matDim,
         vcccddim1 = vcccd.dim;
    if (RHF) {
        oneIntegralMem = (m_norbs / 2 * (m_norbs / 2 + 1)) / 2;
    ...
    long intdim = oneIntegralMem + vccIntegralMem + twoIntegralMem +
                  vccccIntegralMem + vcccdIntegralMem;
    ...
    if (integralIndex == 0) {
        ...
        region = boost::interprocess::mapped_region{
            segment, boost::interprocess::read_write};
        memset(region.get_address(), 0.,
               intdim * m_num_Integrals * sizeof(double));
    }
#ifndef SERIAL
    // wait for all procs to zero out the memory
    MPI::COMM_WORLD.Barrier();
#endif
    v1.set_data() =
        static_cast<double *>(region.get_address()) + intdim * integralIndex;
    vcc.set_data() = static_cast<double *>(region.get_address()) +
                     oneIntegralMem + intdim * integralIndex;
    v2.set_data() = static_cast<double *>(region.get_address()) +
                    oneIntegralMem + vccIntegralMem + intdim * integralIndex;
    vcccc.set_data() = static_cast<double *>(region.get_address()) +
                       oneIntegralMem + vccIntegralMem + twoIntegralMem +
                       intdim * integralIndex;
    vcccd.set_data() = static_cast<double *>(region.get_address()) +
                       oneIntegralMem + vccIntegralMem + twoIntegralMem +
                       vccccIntegralMem + intdim * integralIndex;
    ...
    if (rank == 0) {
        ...
        while (msg.size() != 0) {
            ...
            if (i == -1 && j == -1 && k == -1 && l == -1) {
                ...
                if (section == 0) { // ccdd
                    v2(2 * reorder.at(i), 2 * reorder.at(k), 2 * reorder.at(j),
                       2 * reorder.at(l)) = value;
                } else if (section == 1) { // cccd
                    vcccd.set(2 * reorder.at(i), 2 * reorder.at(j),
                              2 * reorder.at(k) + 1, 2 * reorder.at(l), value);
                } else if (section == 2) { // cccc
                    vcccc.set(2 * reorder.at(i), 2 * reorder.at(j),
                              2 * reorder.at(l) + 1, 2 * reorder.at(k) + 1,
                              value);
                ...
                if (section == 0) { // ccdd_aa
                    ...
                    v2(2 * reorder.at(i) + 1, 2 * reorder.at(k) + 1,
                       2 * reorder.at(j) + 1, 2 * reorder.at(l) + 1) = value;
                } else if (section == 2) { // ccdd_ab
                    v2(2 * reorder.at(i), 2 * reorder.at(k) + 1,
                       2 * reorder.at(j), 2 * reorder.at(l) + 1) = value;
                } else if (section == 3) { // cccd_a
                    vcccd.set(2 * reorder.at(i), 2 * reorder.at(j),
                              2 * reorder.at(k) + 1, 2 * reorder.at(l), value);
                } else if (section == 4) { // cccd_b
                    vcccd.set(2 * reorder.at(i) + 1, 2 * reorder.at(j) + 1,
                              2 * reorder.at(k), 2 * reorder.at(l) + 1, value);
    ...
    MPI::COMM_WORLD.Bcast(v1.set_data() + (maxIter)*maxint,
                          intdim - maxIter * maxint, MPI_DOUBLE, 0);
    MPI::COMM_WORLD.Barrier();

#endif
}

void SpinAdapted::Input::readorbitalsfile(
    string &orbitalfile, OneElectronArray &v1, TwoElectronArray &v2,
    OneElectronArray &vpt1,
    std::map<TwoPerturbType, PerturbTwoElectronArray> &vpt2,
    double &coreEnergy) {
    // TODO
    ...
    if (get_restart() || get_fullrestart()) {
        if (mpigetrank() == 0) {
            ...
            if (!boost::filesystem::exists(ReorderFilePath)) {
                ...
                pout << "================" << endl;
                pout << "The Fiedler routine for finding the orbital ordering "
        ...
        // read the reorder file or calculate the reordering using one of the
        // many options
    ...
    if (mpigetrank() == 0) {
        ...
        vpt2[Va].ReSize(m_total_orbs * 2, m_act_size * 2, m_act_size * 2,
                        m_act_size * 2);
        vpt2[Vi].ReSize(m_act_size * 2, m_act_size * 2, m_total_orbs * 2,
                        m_act_size * 2);
    }
    //  for(auto& i: vpt2)
...
#ifndef SERIAL
    segment.truncate((oneIntegralMem + twoIntegralMem + PerturboneIntegralMem) *
                     m_num_Integrals * sizeof(double));
    region = boost::interprocess::mapped_region{
        segment, boost::interprocess::read_write};
    memset(region.get_address(), 0.,
           (oneIntegralMem + twoIntegralMem + PerturboneIntegralMem) *
               m_num_Integrals * sizeof(double));
    // wait for all procs to zero out the memory
    MPI::COMM_WORLD.Barrier();
    v1.set_data() = static_cast<double *>(region.get_address());
    v2.set_data() =
        static_cast<double *>(region.get_address()) + oneIntegralMem;
    vpt1.set_data() = static_cast<double *>(region.get_address()) +
                      oneIntegralMem + twoIntegralMem;
#else
    // TODO
    // For MPS-SC-NEVPT, different MPS perturbers are calculated independently,
    // through calling block through mpi. Block is serial by itself.
    v1.set_data() =
        new double[(oneIntegralMem + twoIntegralMem + PerturboneIntegralMem) *
                   m_num_Integrals];
    ...
    if (mpigetrank() == 0) {
        ...
        {
            assert(false &&
                   "Partial integral file for mps_nevpt2 is not implemented.");
        }
        ...
        while (msg.size() != 0) {
            ...
            if (i == -1 && j == -1 && k == -1 && l == -1) {
                ...
                v1(2 * reorder.at(i), 2 * reorder.at(j)) = value;
                v1(2 * reorder.at(j), 2 * reorder.at(i)) = value;
            } else {
                int I = reorder.at(i);
                int J = reorder.at(j);
                int K = reorder.at(k);
        ...
        for (int type = 0; type < 2; type++) {
            ...
            while (msg.size() != 0) {
                ...
                if (i == -1 && j == -1 && k == -1 && l == -1 &&
                    ...
                    int K = reorder.at(k);
                    int L = reorder.at(l);
                    vpt2[static_cast<TwoPerturbType>(type)](2 * I, 2 * K, 2 * J,
                                                            2 * L) = value;
                }
                msg.resize(0);
                ReadMeaningfulLine(
    ...
    MPI::COMM_WORLD.Bcast(v1.set_data() + (maxIter)*maxint,
                          oneIntegralMem + twoIntegralMem +
                              PerturboneIntegralMem - maxIter * maxint,
                          MPI_DOUBLE, 0);
    MPI::COMM_WORLD.Barrier();
...
std::vector<int> SpinAdapted::Input::get_fiedler_nevpt(string &dumpname,
                                                       int nact) {
    Matrix fiedler;
    ifstream dumpFile;
    dumpFile.open(dumpname.c_str(), ios::in);
...
std::vector<int>
SpinAdapted::Input::getgaorder(ifstream &gaconfFile, string &orbitalfile,
                               std::vector<int> &fiedlerorder) {
    ifstream dumpFile;
    dumpFile.open(orbitalfile.c_str());
    return genetic::gaordering(gaconfFile, dumpFile, fiedlerorder)
        .Gen()
        .Sequence();
}

std::vector<int>
SpinAdapted::Input::getgaorder_bcs(ifstream &gaconfFile, string &orbitalfile,
                                   std::vector<int> &fiedlerorder) {
    ifstream dumpFile;
    dumpFile.open(orbitalfile.c_str());
    return genetic::gaordering_bcs(gaconfFile, dumpFile, fiedlerorder)
        .Gen()
        .Sequence();
}
...
void SpinAdapted::Input::writeSummaryForMolpro() {
    ...
    pout << m_alpha + m_beta << ":" << m_alpha - m_beta << ":"
         << m_total_symmetry_number.getirrep() + 1 << endl;
    ...
    for (int i = 0; i < m_sweep_iter_schedule.size(); i++) {
        ...
        pout << setw(20) << scientific << setprecision(4)
             << m_sweep_noise_schedule[i] << endl;
    }
    if (m_algorithm_type == TWODOT_TO_ONEDOT)
        pout << setw(50) << "Switching from twodot to onedot algorithm : "
             << m_twodot_to_onedot_iter << endl
             << endl;
    pout << setw(50) << "Maximum sweep iterations : " << m_maxiter << endl
         << endl;
}
#endif
...
void SpinAdapted::Input::writeSummary() {
    ...
    if (rank == 0) {
        ...
        // if (m_Bogoliubov)
        //  printf("%-50s :   even:%-i:%-i\n", "Symmetry of the targeted
        //  wavefunctions", m_alpha - m_beta,
        //  m_total_symmetry_number.getirrep()+1);
        // else
        //  printf("%-50s :   %-i:%-i:%-i\n", "Symmetry of the targeted
        //  wavefunctions",m_alpha + m_beta, m_alpha - m_beta,
        //  m_total_symmetry_number.getirrep()+1);
        // printf("%-50s :   %-i\n", "Number of wavefunctions targeted",
        // m_nroots); if (m_nroots >1) {
        ...
        //  "Davidson_tol",  "Random_noise"); for (int i=0;
        //  i<m_sweep_iter_schedule.size(); i++)
        //    printf("%-10i : %-20i  %-20.4e  %-20.4e\n",
        //    m_sweep_iter_schedule[i], m_sweep_state_schedule[i],
        //    m_sweep_tol_schedule[i], m_sweep_noise_schedule[i]);
        //  if (m_algorithm_type == TWODOT_TO_ONEDOT)
        //    printf("%-50s :   %-i\n", "Switching from twodot to onedot
        //    algorithm", m_twodot_to_onedot_iter);
        //
        ...
        if (m_nroots > 1) {
            pout << setw(50) << left << "The weights of the wavefunctions"
                 << " :   ";
            for (int i = 0; i < m_nroots; ++i)
                pout << left << setprecision(2) << setw(10) << scientific
                     << m_weights[i];
            pout << endl;
        }
        ...
        if (m_algorithm_type == TWODOT_TO_ONEDOT) {
            pout << setw(50) << left
                 << "Switching from twodot to onedot algorithm"
                 << " :   " << m_twodot_to_onedot_iter << endl;
        }
        pout << setw(50) << left << "Maximum sweep iterations"
             << " :   " << m_maxiter << endl;
...
void SpinAdapted::Input::performSanityTest() {
    ...
    if (rank == 0) {
#endif
        if (m_norbs <= 0) {
            pout << "total number of orbitals has to be a positive number"
                 << endl;
            abort();
        }
        ...
        */
        ...
        if (m_alpha < m_beta) {
            pout << "DMRG requires the spin to a positive number and less than "
                    "the total number of electrons"
                 << endl;
            abort();
        }
        if (m_norbs < m_alpha + m_beta) {
            pout << "No of spin orbitals has to be greater than total number "
                    "of electrons"
                 << endl;
            abort();
        }
        if (m_calc_type == RESPONSEAAAC || m_calc_type == RESPONSEAAAV) {
            if (get_sweep_type() == FULL) {
                pout << "Responseaaac and Responseaaav should be used with "
                        "partial sweep"
                     << endl;
                abort();
            }
        ...
        if ((m_calc_type == RESPONSE || m_calc_type == RESPONSELCC ||
             m_calc_type == RESPONSEAAAV || m_calc_type == RESPONSEAAAC ||
             m_calc_type == RESPONSEBW) &&
            m_occupied_orbitals == -1) {
            pout << "For response type of calculation, number of occupied "
                    "orbitals must be specified"
                 << endl;
            abort();
        }
        ...
        if (m_schedule_type_default && !m_schedule_type_backward) {
            if (m_maxM == 0) {
                pout << "With default schedule a non-zero maxM has to be "
                        "specified"
                     << endl;
            ...
            if (m_maxM > 10000) {
                pout << "default schedule only works for maxM less than 10000"
                     << endl;
                abort();
            }
            if (m_maxM <= m_nroots) {
            ...
            if (m_maxM < m_startM) {
                ...
                pout << "Make sure you specify a maxM larger than " << m_startM
                     << endl;
            ...
            if (lastM > m_startM) {
                ...
                pout << "Make sure you specify a lastM larger than " << lastM
                     << endl;
            ...
            if (m_maxM != 0) {
                pout << "With detailed schedule a non-zero maxM should not be "
                        "specified"
                     << endl;
                abort();
            }
        ...
        // if(m_algorithm_type == TWODOT_TO_ONEDOT && m_twodot_to_onedot_iter ==
        // 0)
        //  m_twodot_to_onedot_iter = min(m_sweep_iter_schedule.back()+2,
        //  m_maxiter-1);

        if (m_algorithm_type == TWODOT_TO_ONEDOT &&
            m_twodot_to_onedot_iter >= m_maxiter) {
            pout << "Switch from twodot to onedot algorithm cannot happen "
                    "after maxiter"
                 << endl;
        ...
        if (m_algorithm_type != ONEDOT && m_stateSpecific == true) {
            pout << "Only onedot algorithm is allowed with state specific "
                    "calculation."
                 << endl;
            abort();
        }
...
void SpinAdapted::Input::makeInitialHFGuess() {
    ...
    if (m_Bogoliubov) {
        // overwrite hf_occ_user option, since initial guess is always vacuum in
        // BCS case
        ...
        if (m_hf_occupancy.size() != m_norbs / 2) {
            pout << "ERROR: The length of user-defined HF occupancies does not "
                    "match the number of orbitals "
                 << endl;
            pout << "Length of occupancies is: " << m_hf_occupancy.size()
                 << ", and the number of orbitals is: " << m_norbs / 2 << endl;
            abort();
        }
        int UserElectrons = 0;
        for (int i = 0; i < m_hf_occupancy.size(); i++) {
            UserElectrons += m_hf_occupancy[i];
            if (m_hf_occupancy[i] == 2) {
                hf_occupancy_tmp[2 * i] = 1;
                hf_occupancy_tmp[2 * i + 1] = 1;
            } else if (m_hf_occupancy[i] == 1) {
                hf_occupancy_tmp[2 * i] = 1;
                hf_occupancy_tmp[2 * i + 1] = 0;
            } else if (m_hf_occupancy[i] == 0) {
                hf_occupancy_tmp[2 * i] = 0;
                hf_occupancy_tmp[2 * i + 1] = 0;
            } else {
                pout << "the HF occupancy of orbital " << i
                     << " should either be 2, 1, or 0. Instead "
                     << m_hf_occupancy[i] << " is given." << endl;
                abort();
            }
        }

        if (UserElectrons != m_alpha + m_beta) {
            pout << "ERROR: The total number of electrons in HF occupancy does "
                    "not match the number given initially "
                 << endl;
            pout << "#electron in HF occupancy: " << UserElectrons
                 << ", and the number of electrons given initially: "
                 << m_alpha + m_beta << endl;
            abort();
        }
        ...
        // arrange t(i,i) in a multimap and it will rearrange the ts in
        // ascending order with i
        ...
        pout << "currently other options besides manual, integral  and "
                "canonical are not implemented."
             << endl;
        abort();
    }
...
int SpinAdapted::Input::nroots(int sweep_iter) const {
    ...
    int nroots = m_nroots;
    if (m_noise_type == EXCITEDSTATE &&
        m_sweep_additional_noise_schedule[current] != 0.0 && nroots == 1)
        nroots++;
...
std::vector<double> SpinAdapted::Input::weights(int sweep_iter) const {
    ...
    if (nroots != m_nroots + 1 && nroots != m_nroots) {
        pout << "Something wrong with the number of weights of wavefunctions!"
             << endl;
        exit(0);
    }
    return weights;
