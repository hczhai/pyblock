...
class Input {
  ...
  private:
    ...
    IrrepSpace
        m_bra_symmetry_number; // This is used when bra and ket have different
                               // spatial symmetry irrep; It is only used for
                               // transition density matrix calculations.
    ...
    std::vector<int>
        m_reorder; // this can be manual, fiedler, gaopt or noreorder
    string m_gaconffile;
    ...
    void serialize(Archive &ar, const unsigned int version) {
        ar &m_lowMemoryAlgorithm &m_memory &m_mkl_thrds &m_quanta_thrds
            &m_thrds_per_node &m_spinAdapted &m_Bogoliubov &m_stateSpecific
                &m_implicitTranspose &m_num_Integrals;
        ar &m_norbs &m_partialSweep &m_alpha &m_beta &m_sweep_type &m_solve_type
            &m_Sz &m_set_Sz &m_baseState &m_projectorState &m_targetState;
        ar &m_spin_vector &m_spin_orbs_symmetry &m_guess_permutations &m_nroots
            &m_weights &m_hf_occ_user &m_hf_occupancy;
        ar &m_sweep_iter_schedule &m_sweep_state_schedule
            &m_sweep_qstate_schedule &m_sweep_tol_schedule
                &m_sweep_noise_schedule &m_sweep_additional_noise_schedule
                    &m_reorder;
        ar &m_molecule_quantum &m_total_symmetry_number &m_total_spin
            &m_orbenergies &m_add_noninteracting_orbs;
        ar &m_bra_symmetry_number &m_permSymm &m_activeorbs &m_excitation
            &m_openorbs &m_closedorbs;
        ar &m_save_prefix &m_load_prefix &m_direct &m_max_lanczos_dimension
            &m_performResponseSolution;
        ...
        ar &m_do_pdm &m_do_npdm_ops &m_do_npdm_in_core &m_npdm_generate
            &m_new_npdm_code &m_specificpdm &m_transition_diff_spatial_irrep
                &m_occupied_orbitals;
        ar &m_store_spinpdm &m_spatpdm_disk_dump &m_pdm_unsorted
            &m_npdm_intermediate &m_npdm_multinode;
        ar &m_maxj &m_ninej &m_maxiter &m_do_deriv &m_oneindex_screen_tol
            &m_twoindex_screen_tol &m_quantaToKeep &m_noise_type;
        ar &m_sweep_tol &m_restart &m_backward &m_fullrestart &m_restart_warm
            &m_reset_iterations &m_calc_type &m_ham_type &m_warmup;
        ar &m_do_diis &m_diis_error &m_start_diis_iter &m_diis_keep_states
            &m_diis_error_tol &m_num_spatial_orbs;
        ar &m_spatial_to_spin &m_spin_to_spatial &m_maxM &m_bra_M
            &m_schedule_type_backward &m_schedule_type_default
                &m_integral_disk_storage_thresh;
        ar &n_twodot_noise &m_twodot_noise &m_twodot_gamma &m_guessState
            &m_useSharedMemory;
        ar &m_calc_ri_4pdm &m_store_ripdm_readable &m_nevpt2
            &m_conventional_nevpt2 &m_kept_nevpt2_states &NevPrint;
        ar &m_act_size &m_core_size &m_virt_size &m_total_orbs
            &m_total_spin_orbs_symmetry &m_total_spatial_to_spin
                &m_total_spin_to_spatial;
    }
  ...
  public:
    ...
    Input() {}
    Input(const std::string &config_name, const std::string &contents = "");
    // ROA
    int matmultNum;
    ...
    void readorbitalsfile(string &dumpFile, OneElectronArray &v1,
                          TwoElectronArray &v2, double &coreEnergy,
                          int integralIndex);
    void readorbitalsfile(string &dumpFile, OneElectronArray &v1,
                          TwoElectronArray &v2, double &coreEnergy,
                          PairArray &vcc, CCCCArray &vcccc, CCCDArray &vcccd,
                          int integralIndex);
    void
    readorbitalsfile(string &orbitalfile, OneElectronArray &v1,
                     TwoElectronArray &v2, OneElectronArray &vpt1,
    ...
    std::vector<int> getgaorder(ifstream &gaconfFile, string &orbitalfile,
                                std::vector<int> &fiedlerorder);
    std::vector<int> getgaorder_bcs(ifstream &gaconfFile, string &orbitalfile,
                                    std::vector<int> &fiedlerorder);
    ...
    static void ReadMeaningfulLine(istream &, string &, int);
    ...
    bool use_partial_two_integrals() const {
        return (m_norbs / 2 >= m_integral_disk_storage_thresh);
    }
    ...
    std::vector<std::map<SpinQuantum, int>> &get_quantaToKeep() {
        return m_quantaToKeep;
    }
    ...
    const std::vector<int> &sweep_iter_schedule() const {
        return m_sweep_iter_schedule;
    }
    const std::vector<int> &sweep_state_schedule() const {
        return m_sweep_state_schedule;
    }
    const std::vector<int> &sweep_qstate_schedule() const {
        return m_sweep_qstate_schedule;
    }
    const std::vector<double> &sweep_tol_schedule() const {
        return m_sweep_tol_schedule;
    }
    const std::vector<double> &sweep_noise_schedule() const {
        return m_sweep_noise_schedule;
    }
    const std::vector<double> &sweep_additional_noise_schedule() const {
        return m_sweep_additional_noise_schedule;
    }
    std::vector<double> &set_sweep_noise_schedule() {
        return m_sweep_noise_schedule;
    }
    std::vector<double> &set_sweep_additional_noise_schedule() {
        return m_sweep_additional_noise_schedule;
    }
    int &Sz() { return m_Sz; }
    ...
    int total_particle_number() const {
    ...
    void SetPrint(bool p, int i = 0) {
        NevPrint.first = p;
        NevPrint.second = i;
    }
    const SpinSpace total_spin_number() const {
        if (!m_add_noninteracting_orbs)
            return SpinSpace(m_alpha - m_beta);
        else
            return SpinSpace(0);
    }
    int last_site() const {
    ...
    const IrrepSpace &total_symmetry_number() const {
        return m_total_symmetry_number;
    }
    const IrrepSpace &bra_symmetry_number() const {
        return m_bra_symmetry_number;
    }
    const SpinQuantum &molecule_quantum() const { return m_molecule_quantum; }
    const int &sys_add() const { return m_sys_add; }
    const bool &add_noninteracting_orbs() const {
        return m_add_noninteracting_orbs;
    }
    ...
    SpinQuantum effective_molecule_quantum() {
        if (!m_add_noninteracting_orbs)
            ...
            return SpinQuantum(m_molecule_quantum.particleNumber +
                                   m_molecule_quantum.totalSpin.getirrep(),
                               SpinSpace(0),
                               m_molecule_quantum.orbitalSymmetry);
        // return SpinQuantum(total_particle_number() +
        // total_spin_number().getirrep(), SpinSpace(0),
        // total_symmetry_number());
    }
    SpinQuantum bra_quantum() {
        if (!m_add_noninteracting_orbs)
            return SpinQuantum(total_particle_number(),
                               SpinSpace(m_alpha - m_beta),
                               bra_symmetry_number());
        else
            return SpinQuantum(total_particle_number() +
                                   total_spin_number().getirrep(),
                               SpinSpace(0), bra_symmetry_number());
    }
    ...
    bool transition_diff_irrep() { return m_transition_diff_spatial_irrep; }
    ...
    const bool &store_nonredundant_pdm() const {
        return m_store_nonredundant_pdm;
    }
