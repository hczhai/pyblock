...
SpinAdapted::Csf::Csf(const map<Slater, double> &p_dets, const int p_n,
                      const SpinSpace p_S, const int p_Sz,
                      const IrrepVector p_irrep)
    : det_rep(p_dets), n(p_n), S(p_S), Sz(p_Sz), irrep(p_irrep) {
    ...
    for (; it != det_rep.end(); it++)
        if ((*it).first.n != n || (*it).first.Sz != Sz) {
            ...
            pout << " all slaters in Csf do not have the same n, spin or "
                    "symmetry"
                 << endl;
            pout << *this << endl;
            throw 20;
...
void SpinAdapted::Csf::outerProd(const Csf &csf, double factor,
                                 map<Slater, double> &output) const {
    ...
    for (int j = 0; j < 2; j++) {
        ...
        if (j == 0)
            ...
            vector<vector<int>> partitions =
                CSFUTIL::generate_partitions(irrep.getirrep());
            if (partitions.size() == 0) {
                pout << "big trouble" << endl;
                exit(0);
            }
            for (int vec = 0; vec < partitions.size(); vec++) {
                ...
                if (vec == 0)
                    ...
                    for (map<Slater, double>::iterator it2 = s.det_rep.begin();
                         it2 != s.det_rep.end(); it2++) {
                        map<Slater, double>::iterator itout =
                            tempmap.find(it2->first);
                        if (itout == tempmap.end())
                            tempmap[it2->first] = it2->second;
                        else
                            tempmap[it2->first] += it2->second;
                    }
                    sLminus.set_det_rep(tempmap, S,
                                        IrrepVector(this->irrep.getirrep(), 0));
                }
            }
            sLminus.normalize();
            ...
            continue;
        for (int i = this->S.getirrep() - 2; i >= this->S.getirrep() - 2 * k;
             i -= 2) {
            Csf sm;
            s.applySminus(sm);
            if (sm.size() != 0)
...
void SpinAdapted::CSFUTIL::TensorProduct(Csf &lhs, vector<Csf> &lhs_csfs,
                                         Csf &rhs, vector<Csf> &rhs_csfs,
                                         vector<Csf> &output,
                                         vector<vector<Csf>> &outputladder) {
        ...
        for (int j = 0; j < spinvec.size(); j++) {
            ...
            for (int row = 0; row < Symmetry::sizeofIrrep(irrep); row++)
                for (int M = -J; M <= J; M += 2) {
                    ...
                    for (int j1m1 = 0; j1m1 < lhs_csfs.size();
                         j1m1++) // loops over rows of J1 lirrep
                        for (int j2m2 = 0; j2m2 < rhs_csfs.size();
                             j2m2++) // loops over rows of J2 rirrep
                        {
                            int J1 = lhs.S.getirrep(), M1 = lhs_csfs[j1m1].Sz,
                                J2 = rhs.S.getirrep(), M2 = rhs_csfs[j2m2].Sz;
                            int rowl = lhs_csfs[j1m1].irrep.getrow(),
                                rowr = rhs_csfs[j2m2].irrep.getrow();

                            double clebsg = cg(J1, J2, J, M1, M2, M);
                            double clebspatial = Symmetry::spatial_cg(
                                lirrep, rirrep, irrep, rowl, rowr, row);
                            if (abs(clebsg) < 1.0e-12 ||
                                abs(clebspatial) < 1.00e-14)
                                continue;
                            lhs_csfs[j1m1].outerProd(
                                rhs_csfs[j2m2], clebsg * clebspatial, dets);
                        }

                    if (dets.size() == 0)
                        continue;
                    Csf c(dets, rhs.n + lhs.n, spinvec[j], M,
                          IrrepVector(symvec[l].getirrep(), row));
                    c.normalize();
                    if (M == J && row == 0)
                        output.push_back(c);
...
void SpinAdapted::CSFUTIL::TensorProduct(Csf &lhs, Csf &rhs,
                                         vector<Csf> &output) {
    // vector< Csf> lhs_csfs = lhs.spinLadder(lhs.S);
    // vector< Csf> rhs_csfs = rhs.spinLadder(rhs.S);
    ...
    for (int l = 0; l < symvec.size(); l++)
        for (int j = 0; j < spinvec.size(); j++) {
            ...
            for (int row = 0; row < Symmetry::sizeofIrrep(irrep); row++) {
                ...
                Csf c(dets, rhs.n + lhs.n, spinvec[j], J,
                      IrrepVector(symvec[l].getirrep(), row));
                c.normalize();
...
Csf SpinAdapted::CSFUTIL::applyTensorOp(const TensorOp &newop, int spinL) {
    ...
    // {
    map<Slater, double> m;
    SpinQuantum sq(newop.optypes.size(), SpinSpace(newop.Spin),
                   IrrepSpace(newop.irrep));
    ...
    Csf csf = Csf(m, sq.particleNumber, sq.totalSpin, sz,
                  IrrepVector(sq.get_symm().getirrep(), irreprow));
...
std::vector<SpinAdapted::Csf>
SpinAdapted::CSFUTIL::spinfockstrings(const std::vector<int> &orbs,
    ...
    for (int i = 0; i < orbs.size(); i++) {
        ...
        Slater s1(occ_rep1, 1), s2(occ_rep2, 1);
        map<Slater, double> m1, m2;
        m1[s1] = 1.0;
        m2[s2] = 1.0;

        if ((dmrginp.calc_type() != RESPONSELCC &&
             dmrginp.calc_type() != RESPONSEAAAV &&
             dmrginp.calc_type() != RESPONSEAAAC) &&
            find(dmrginp.get_openorbs().begin(), dmrginp.get_openorbs().end(),
                 I) != dmrginp.get_openorbs().end()) {
            thisSiteCsf.push_back(
                Csf(m1, 0, SpinSpace(0), 0, IrrepVector(0, 0))); // 0,0,0
            ladderentry.push_back(
                Csf(m1, 0, SpinSpace(0), 0, IrrepVector(0, 0)));
            ...
            continue;
        } else if ((dmrginp.calc_type() != RESPONSELCC &&
                    dmrginp.calc_type() != RESPONSEAAAV &&
                    dmrginp.calc_type() != RESPONSEAAAC) &&
                   find(dmrginp.get_closedorbs().begin(),
                        dmrginp.get_closedorbs().end(),
                        I) != dmrginp.get_closedorbs().end()) {
            std::vector<bool> occ_rep(Slater().size(), 0);
            ...
            m[s] = 1.0;
            thisSiteCsf.push_back(
                Csf(m, 2, SpinSpace(0), 0, IrrepVector(0, 0))); // 2,0,0
            ladderentry.push_back(
                Csf(m, 2, SpinSpace(0), 0, IrrepVector(0, 0)));
            singleSiteLadder.push_back(ladderentry);
            numcsfs.push_back(thisSiteCsf.size());
            ...
            thisSiteCsf.push_back(
                Csf(m2, 1, SpinSpace(1), 1,
                    IrrepVector(Irrep.getirrep(), irrepsize - 1))); // 1,1,L
        ...
        thisSiteCsf.push_back(
            Csf(m2, 1, SpinSpace(1), 1,
                IrrepVector(Irrep.getirrep(), irrepsize - 1))); // 1,1,L
        ladderentry.push_back(Csf(m1, 0, SpinSpace(0), 0, IrrepVector(0, 0)));
        singleSiteLadder.push_back(ladderentry);
        ladderentry.clear();
        ...
        for (int nele = 2; nele < 2 * irrepsize + 1; nele++) {
            ...
            for (int i = 0; i < tensorops.size(); i++) {
                quanta = SpinQuantum(nele - 1, SpinSpace(tensorops[i].Spin),
                                     IrrepSpace(tensorops[i].irrep)) +
                         sQ;

                for (int j = 0; j < quanta.size(); j++) {
                    TensorOp newop = TensorOp(I, 1).product(
                        tensorops[i], quanta[j].totalSpin.getirrep(),
                        quanta[j].get_symm().getirrep());

                    Csf csf = applyTensorOp(newop, newop.Szops.size() - 1 -
                                                       newop.Spin);

                    if (!csf.isempty() && csf.norm() > 1e-10) {
                        csf.normalize();

                        if (find(thisSiteCsf.begin(), thisSiteCsf.end(), csf) ==
                            thisSiteCsf.end()) {
    ...
    for (int i2 = 1; i2 < orbs.size(); i2++) {
        ...
        for (int j = 0; j < prevoutput.size(); j++) {
            for (int k = 0; k < numcsfs[i2]; k++) {
                CSFUTIL::TensorProduct(
                    prevoutput[j], prevladder[j], singleSiteCsf[csfindex + k],
                    singleSiteLadder[csfindex + k], output, ladder);
            }
        }
...
SpinAdapted::CSFUTIL::spinfockstrings(const std::vector<int> &orbs) {
    ...
    for (int i = 0; i < orbs.size(); i++) {
        ...
        std::vector<bool> occ_rep1(Slater().size(), 0),
            occ_rep2(Slater().size(), 0), occ_rep3(Slater().size(), 0),
            occ_rep4(Slater().size(), 0);
        occ_rep2[dmrginp.spatial_to_spin()[I]] = 1;
        occ_rep3[dmrginp.spatial_to_spin()[I] + 1] = 1;
        occ_rep4[dmrginp.spatial_to_spin()[I]] = 1;
        occ_rep4[dmrginp.spatial_to_spin()[I] + 1] = 1;

        Slater s1(occ_rep1, 1), s2(occ_rep2, 1), s3(occ_rep3, 1),
            s4(occ_rep4, 1);
        map<Slater, double> m1, m2, m3, m4;
        m1[s1] = 1.0;
        m2[s2] = 1.0;
        m3[s3] = 1.0;
        m4[s4] = 1.0;
        ...
        thisSiteCsf.push_back(
            Csf(m2, 1, SpinSpace(1), 1,
                IrrepVector(Irrep.getirrep(), irrepsize - 1))); // 1,1,L
        thisSiteCsf.push_back(
            Csf(m3, 1, SpinSpace(-1), -1,
                IrrepVector(Irrep.getirrep(), irrepsize - 1))); // 1,1,L
        thisSiteCsf.push_back(
            Csf(m4, 2, SpinSpace(0), 0, IrrepVector(0, 0))); // 2,0,0
    ...
    for (int i2 = 1; i2 < orbs.size(); i2++) {
        ...
        for (int j = 0; j < prevoutput.size(); j++) {
            for (int k = 0; k < numcsfs[i2]; k++) {
                CSFUTIL::TensorProduct(prevoutput[j],
                                       singleSiteCsf[csfindex + k], output);
            }
        }
...
std::vector<SpinAdapted::Csf>
SpinAdapted::Csf::distribute(const int n, const int sp, const IrrepVector &sym,
                             const int left, const int right, const int edge,
                             int integralIndex) {
    std::vector<Csf> s;
    ...
    if (dmrginp.hamiltonian() == HEISENBERG && nb != 0)
        return s;
    // let's count how many left and right orbitals there actually are
    int actualOrbs =
        dmrginp.spatial_to_spin()[right] - dmrginp.spatial_to_spin()[left];
    ...
    std::vector<std::vector<int>> abList;
    ...
    while (doAgain && numtries < 500) {
        while (next_permutation(abSeed.begin(), abSeed.end()) &&
               numberOfGuesses--)
            abList.push_back(abSeed);
        ...
        // scan orbitals from left to right, pick out orbitals which have
        // smallest v_1(i, i) integrals
        ...
        for (int i = left; i < right; ++i) {
            ...
            orbMap.insert(pair<double, int>(v_1[integralIndex](2 * i, 2 * i),
                                            alphaIndex));
            ++alphaIndex;
        }

        for (multimap<double, int>::iterator m = orbMap.begin();
             m != orbMap.end(); ++m)
        ...
        for (int i = 0; i < abList.size(); ++i) {
            ...
            std::vector<bool> rbuffer(dmrginp.spatial_to_spin()[edge] -
                                          dmrginp.spatial_to_spin()[right],
                                      0);

            std::vector<bool> orbs(dmrginp.spatial_to_spin()[right] -
                                       dmrginp.spatial_to_spin()[left],
                                   0);
            int alphaI = 0;
            int betaI = 0;
            ...
            for (std::vector<bool>::iterator it = orbs.begin();
                 it != orbs.end(); ++it)
                tmp.push_back(*it);
            for (std::vector<bool>::iterator it = rbuffer.begin();
                 it != rbuffer.end(); ++it)
                tmp.push_back(*it);
            ...
            if (sym.getirrep() == AbelianSymmetryOf(new_det).getirrep())
                s.push_back(Csf(m, n, SpinSpace(sp), sp,
                                IrrepVector(sym.getirrep(), 0)));
        }
...
std::vector<Csf> Csf::distributeNonSpinAdapted(const int n, const int sp,
                                               const IrrepVector &sym,
    ...
    // okay now make some permutations (at most GUESS_PERMUTATIONS)
    std::vector<std::vector<int>> alphaList;
    alphaList.push_back(alphaSeed);
    std::vector<std::vector<int>> betaList;
    betaList.push_back(betaSeed);
    int numberOfGuesses = dmrginp.guess_permutations();
    while (next_permutation(alphaSeed.begin(), alphaSeed.end()) &&
           numberOfGuesses--)
        alphaList.push_back(alphaSeed);
    numberOfGuesses = dmrginp.guess_permutations();
    ;
    while (next_permutation(betaSeed.begin(), betaSeed.end()) &&
           numberOfGuesses--)
        betaList.push_back(betaSeed);
    ...
    // scan orbitals from left to right, pick out orbitals which are occupied in
    ...
    for (int i = left; i < right; ++i) {
        ...
        {
            alphaMap.insert(
                pair<double, int>(v_1[integralIndex](i, i), alphaIndex));
            ++alphaIndex;
        } else // beta orb
        {
            betaMap.insert(
                pair<double, int>(v_1[integralIndex](i, i), betaIndex));
            ++betaIndex;
        }
    }

    for (multimap<double, int>::iterator m = alphaMap.begin();
         m != alphaMap.end(); ++m)
        orbitalAlphaList.push_back(m->second);
    for (multimap<double, int>::iterator m = betaMap.begin();
         m != betaMap.end(); ++m)
...
vector<vector<int>> SpinAdapted::CSFUTIL::generate_partitions(int n) {
    ...
    if (Symmetry::sizeofIrrep(n) == 1) {
        pout << "cannot generate partition of irrep which a single row "
             << endl;
        exit(0);
    }
